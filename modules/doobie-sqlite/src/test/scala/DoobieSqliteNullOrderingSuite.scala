...
//   (testdata/sqlite/null-ordering.sql) with NULLs in the ordered column.
...
//
// SQLite sorts NULLs low by default (first in ASC, last in DESC), so the two cases requesting
// the opposite placement only work if the dialect emits an explicit NULLS FIRST/LAST.
...
final class NullOrderingSuite extends DoobieSqliteDatabaseSuite {
  lazy val mapping = new DoobieSqliteTestMapping(transactor) with NullOrderingMapping[IO]
  ...
  test("ascending, nulls last (non-default for SQLite)") {
    assertIO(surviving(ascending = true, nullsLast = true), List(3, 1))
  }

  test("descending, nulls first (non-default for SQLite)") {
    assertIO(surviving(ascending = false, nullsLast = false), List(2, 4))
  }

  test("ascending, nulls first (SQLite's default placement)") {
    assertIO(surviving(ascending = true, nullsLast = false), List(2, 4))
  }

  test("descending, nulls last (SQLite's default placement)") {
