...
trait DoobieSqliteDatabaseSuite extends DoobieDatabaseSuite {
  abstract class DoobieSqliteTestMapping[F[_]: Sync](
    ...
    // The generated scripts spell offsets as e.g. '2020-05-27 21:00:00 +02:00' (space-separated,
    // not the 'T'-separated ISO_OFFSET_DATE_TIME OffsetDateTime.parse defaults to), which is
    // what Oracle and SQL Server read too - so a custom formatter is used here rather than
    // giving SQLite a spelling of its own.
  ...
  // Where the generated scripts live - see the `Test / javaOptions` setting for
  // grackle-doobie-sqlite in build.sbt, which points this at target/testdata/sqlite/ regardless
  // of the fork's working directory.
