...
object MovieData {
  ...
  object Movie {
    implicit val cursorBuilder: CursorBuilder[Movie] =
      deriveObjectCursorBuilder[Movie]
  }
...
object MovieQueryInterpreter extends GenericQueryInterpreter[Id](
  {
    case "movieById"|"moviesByGenre"|"moviesReleasedBetween"|"moviesLongerThan"|"moviesShownLaterThan"|"moviesShownBetween" =>
      CursorBuilder[List[Movie]].build(movies, ListType(MovieType))
  }
)
