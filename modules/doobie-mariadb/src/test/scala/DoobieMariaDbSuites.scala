...
final class ErrorKeysSuite extends DoobieMariaDbDatabaseSuite with SqlErrorKeysSuite {
  lazy val mapping = new DoobieMariaDbTestMapping(transactor) with SqlCompositeKeyMapping[IO]
}
...
final class NullableParentSuite extends DoobieMariaDbDatabaseSuite with SqlNullableParentSuite {
  lazy val mapping = new DoobieMariaDbTestMapping(transactor) with SqlNullableParentMapping[IO]
}

final class NullOrderingSuite extends DoobieMariaDbDatabaseSuite with SqlNullOrderingSuite {
  lazy val mapping = new DoobieMariaDbTestMapping(transactor) with SqlNullOrderingMapping[IO]
}
...
final class QualifiedNamesSuite extends DoobieMariaDbDatabaseSuite with SqlQualifiedNamesSuite {
  lazy val mapping = new DoobieMariaDbTestMapping(transactor) with SqlQualifiedNamesMapping[IO]
}

final class TableNameSuite extends DoobieMariaDbDatabaseSuite with SqlTableNameSuite {
  lazy val mapping = new DoobieMariaDbTestMapping(transactor) with SqlQualifiedNamesMapping[IO]
}
...
final class UnionOrderSuite extends DoobieMariaDbDatabaseSuite with SqlUnionOrderSuite {
  lazy val mapping = new DoobieMariaDbTestMapping(transactor) with SqlUnionOrderMapping[IO]
}
...
/**
 * Pins the rendering of an ascribed null for a driver-native JSON codec.
 *
 * Nothing else in this module reaches that branch: the suites' `jsonb` codec is built from
 * `Meta[String]`, so `sqlTypeName` reports VARCHAR for it and the JSON case never fires.
 * MariaDB has to render a bare NULL here, `CAST(NULL AS JSON)` being a syntax error on it,
 * where MySQL renders the cast - the two dialects disagree by design and a test each is what
 * says so.
 */
final class AscribedJsonNullSuite extends DoobieMariaDbDatabaseSuite {
  test("ascribed null for a JSON column") {
    val json: Meta[String] =
      new Meta(
        Get.Advanced.other[String](NonEmptyList.of("JSON")),
        Put.Advanced.other[String](NonEmptyList.of("JSON")))
    val mapping = new DoobieMariaDbTestMapping(transactor) with SqlCompositeKeyMapping[IO]
    val sql = mapping.ascribedNullToFragment((json, false)).internals.sql
    assertEquals(sql.trim, "NULL")
  }
}
