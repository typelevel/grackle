...
//   (testdata/mysql/null-ordering.sql) with NULLs in the ordered column.
...
//
// MySQL sorts NULLs low by default (first in ASC, last in DESC), same as SQLite, so the two
// cases requesting the opposite placement only work if the dialect emulates NULLS FIRST/LAST.
...
final class NullOrderingSuite extends DoobieMariaDbDatabaseSuite {
  lazy val mapping = new DoobieMariaDbTestMapping(transactor) with NullOrderingMapping[IO]
  ...
  test("ascending, nulls last (non-default for MySQL)") {
    assertIO(surviving(ascending = true, nullsLast = true), List(3, 1))
  }

  test("descending, nulls first (non-default for MySQL)") {
    assertIO(surviving(ascending = false, nullsLast = false), List(2, 4))
  }

  test("ascending, nulls first (MySQL's default placement)") {
    assertIO(surviving(ascending = true, nullsLast = false), List(2, 4))
  }

  test("descending, nulls last (MySQL's default placement)") {
