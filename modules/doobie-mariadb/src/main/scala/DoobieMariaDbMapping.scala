 ...
 * MariaDB dialect.
 *
 * Unlike MySQL, MariaDB has no LATERAL derived tables in any released version, so mkLateral
 * answers NotLateral (SqlMappingLike derives supportsLateralJoin = false from that, the same
 * mechanism SQLite/H2 use - see mkLateral's comment below for the consequences). Everything
 * else - no NULLS FIRST/LAST (CASE-WHEN emulation, nulls-low polarity), no DISTINCT ON
 * (FirstValueColumn), no ILIKE (LIKE is already case-insensitive under the default *_ci
...
trait DoobieMariaDbMappingLike[F[_]] extends DoobieMappingLike[F] with SqlMappingLike[F] {
  ...
  // The collation MariaDB needs is applied in the ORDER BY and LIKE fragments themselves, so a
