    ...
    List(
      ObjectMapping(
        ...
        fieldMappings =
          List(
            SkunkRoot("movieById"),
            SkunkRoot("moviesByGenre"),
            SkunkRoot("moviesReleasedBetween"),
            SkunkRoot("moviesLongerThan"),
            SkunkRoot("moviesShownLaterThan"),
            SkunkRoot("moviesShownBetween"),
            SkunkRoot("longMovies")
          )
      ),
      ObjectMapping(
        tpe = MovieType,
        fieldMappings =
          List(
            SkunkField("id", ColumnRef("movies", "id", uuid), key = true),
            SkunkField("title", ColumnRef("movies", "title", text)),
            SkunkField("genre", ColumnRef("movies", "genre", Genre.codec)),
            SkunkField("releaseDate", ColumnRef("movies", "releasedate", date)),
            SkunkField("showTime", ColumnRef("movies", "showtime", time)),
            SkunkField("nextShowing", ColumnRef("movies", "nextshowing", timestamptz)),
            CursorField("nextEnding", nextEnding, List("nextShowing", "duration")),
            SkunkField("duration", ColumnRef("movies", "duration", int8.imap(Duration.ofMillis)(_.toMillis))),
            // SkunkField("categories", ColumnRef("movies", "categories")),
            // SkunkField("features", ColumnRef("movies", "features")),
            // SkunkField("rating", ColumnRef("movies", "rating")),
            CursorAttribute("isLong", isLong, List("duration"))
          )
      ),
      SkunkLeafMapping[UUID](UUIDType, uuid),
      SkunkLeafMapping[LocalTime](TimeType, time),
      SkunkLeafMapping[LocalDate](DateType, date),
      SkunkLeafMapping[OffsetDateTime](DateTimeType, timestamptz),
      SkunkLeafMapping[Duration](IntervalType, int8.imap(Duration.ofMillis)(_.toMillis)),
      SkunkLeafMapping[Genre](GenreType, Genre.codec),
      LeafMapping[Feature](FeatureType),
      // SkunkLeafMapping[List[Feature]](ListType(FeatureType))
    )
