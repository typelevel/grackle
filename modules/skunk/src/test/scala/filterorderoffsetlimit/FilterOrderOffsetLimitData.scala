...
trait FilterOrderOffsetLimitMapping[F[_]] extends SkunkMapping[F] {
...
object FilterOrderOffsetLimitMapping extends SkunkMappingCompanion {
  def mkMapping[F[_]: Sync](pool: Resource[F, Session[F]], monitor: SkunkMonitor[F]): FilterOrderOffsetLimitMapping[F] =
    new SkunkMapping(pool, monitor) with FilterOrderOffsetLimitMapping[F]
}
