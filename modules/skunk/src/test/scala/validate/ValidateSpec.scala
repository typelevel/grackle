...
final class ValidateSpec extends AnyFunSuite {
  ...
  abstract class TestMapping extends SkunkMapping[IO](null, SkunkMonitor.noopMonitor) {
    ...
    // Error messages for types that appear in the schema but are unmapped.
    def unmappedTypes: List[SchemaValidationFailure] =
      schema.types.filterNot(tpe => typeMappings.exists(_.tpe =:= tpe)).map { tpe =>
        SchemaValidationFailure.noMapping(tpe)
      }

    lazy val validationFailures: Chain[SchemaValidationFailure] =
      Chain.fromSeq(unmappedTypes) ++
      Chain.fromSeq(typeMappings).foldMap(_.validationFailures)
  ...
  test("fails") {
    ...
    object Mapping1 extends TestMapping {
      ...
      val typeMappings =
        List(
          ...
          LeafMapping[String](UUIDType), // this is the wrong schema
    ...
    Mapping1.validationFailures.toList.sortBy(_.severity).foreach { f =>
      println("---")
      println(f.toErrorMessage)
    }
