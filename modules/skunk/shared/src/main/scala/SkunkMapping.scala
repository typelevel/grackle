...
import skunk.{AppliedFragment, Decoder, Fragment => SFragment, Session}
import skunk.codec.all.{bool, float8, int4, varchar}
...
trait SkunkMappingLike[F[_]] extends Mapping[F] with SqlPgMappingLike[F] { outer =>
  ...
  type Codec = (skunk.Codec[_], Boolean)
  type Encoder = skunk.Encoder[_]
  ...
  object IsNullable extends IsNullable0 {
    implicit def nullable[T]: IsNullable[Option[T]] = new IsNullable[Option[T]] {
      def isNullable = true
    }
  }
  trait IsNullable0 {
  ...
  object NullableTypeName extends NullableTypeName0 {
    implicit def nullable[T](implicit typeName: TypeName[T]): NullableTypeName[Option[T]] =
      new NullableTypeName[Option[T]] { def value = typeName.value }
  }
  trait NullableTypeName0 {
    implicit def notNullable[T](implicit typeName: TypeName[T]): NullableTypeName[T] =
      new NullableTypeName[T] { def value = typeName.value }
  }
  ...
  implicit def Fragments: SqlFragment[AppliedFragment] =
    new SqlFragment[AppliedFragment] { self =>
      ...
      def bind[A](encoder: Encoder, value: A) =
        sql"$encoder".asInstanceOf[SFragment[A]].apply(value)
      def in[G[_]: Reducible, A](f: AppliedFragment, fs: G[A], enc: Encoder): AppliedFragment =
        fs.toList
          .map(sql"$enc".asInstanceOf[SFragment[A]].apply)
          .foldSmash(f |+| void" IN (", void", ", void")")(self, Foldable[List])

      def const(s: String) = sql"#$s".apply(skunk.Void)
      def and(fs: AppliedFragment*): AppliedFragment =
        fs.toList.map(parentheses).intercalate(void" AND ")(self)
      def andOpt(fs: Option[AppliedFragment]*): AppliedFragment = and(fs.toList.unite: _*)
      def or(fs: AppliedFragment*): AppliedFragment =
        fs.toList.map(parentheses).intercalate(void" OR ")(self)
      ...
      def whereAndOpt(fs: Option[AppliedFragment]*): AppliedFragment = whereAnd(
        fs.toList.unite: _*)
  ...
  def fetch(fragment: Fragment, codecs: List[(Boolean, Codec)]): F[Vector[Array[Any]]] = {
    ...
    pool
      ...
      .onError { case NonFatal(e) => Sync[F].delay(e.printStackTrace()) }
  }
}
