...
trait SqlMappingLike[F[_]] extends CirceMappingLike[F] with SqlModule[F] { self =>
  ...
  import TableExpr.{DerivedTableRef, Laterality, SubqueryRef, TableRef, WithRef}
  ...
  /**
   * Renders one branch of a `UNION ALL` compound select, typically by wrapping it in
   * parentheses: `(branch1) UNION ALL (branch2)`. Dialects whose compound-select grammar
   * forbids parenthesized branches (e.g. SQLite) return the fragment unmodified instead. That
   * is safe for any dialect: grackle only ever combines branches with `UNION ALL` (never plain
   * `UNION`), which is associative, so grouping never affects results. A branch that carries
   * its own `ORDER BY`/`OFFSET`/`LIMIT` is wrapped in a derived-table subquery by
   * `encapsulateUnionBranch` before this is applied.
   */
  ...
  /**
   * Whether this dialect can correlate a FROM-clause subquery with a sibling table (`LATERAL`,
   * `CROSS`/`OUTER APPLY`, or equivalent). Derived from `mkLateral`: a dialect with no such
   * mechanism at all (e.g. SQLite) has no lateral form for `mkLateral` to render and answers
   * `Laterality.NotLateral` there, which is what this test detects.
   *
  ...
  /**
   * Name of a SQL schema column and its associated codec, Scala type an defining source
   * position within an `SqlMapping`.
   *
   * `Column`s are considered equal if their table and column names are equal.
   *
   * Note that `ColumnRef` primarily play a role in mappings. During compilation they will be
  ...
  object Aliased {
    ...
    def columnDef(column: SqlColumn): Aliased[(Option[String], String)] = StateT(
      _.columnDef(column).success)
    def columnRef(column: SqlColumn): Aliased[(Option[String], String)] = StateT(
      _.columnRef(column).success)
    ...
    def internalError[T](msg: String): Aliased[T] =
      StateT(_ => Result.internalError[(AliasState, T)](msg))
    def internalError[T](err: Throwable): Aliased[T] =
      StateT(_ => Result.internalError[(AliasState, T)](err))
  }

  /**
   * State required to assign table and column aliases.
   *
   * Used when rendering an `SqlQuery` as a `Fragment`. Table aliases are assigned as needed for
   * recursive queries. Column aliases are assigned to disambiguate collections of columns
   * generated by subqueries and unions.
   */
  case class AliasState(
    ...
    def columnDef(column: SqlColumn): (AliasState, (Option[String], String)) = {
      val (newState0, table0) =
        column.namedOwner.map(named => tableDef(named).fmap(Option(_))).getOrElse((this, None))
      columnAliases.get(
        (column.underlying.owner.context.resultPath, column.underlying.column)) match {
        ...
        case None =>
          val (next0, seenColumns0, column0) =
            if (newState0.seenColumns(column.column))
              (
                newState0.next + 1,
                newState0.seenColumns,
                s"${column.column}_alias_${newState0.next}")
            else
              (newState0.next, newState0.seenColumns + column.column, column.column)

          val columnAliases0 =
            newState0.columnAliases + ((
              column.underlying.owner.context.resultPath,
              column.underlying.column) -> column0)

          val newState = newState0.copy(
            next = next0,
            seenColumns = seenColumns0,
            columnAliases = columnAliases0)
    ...
    def columnRef(column: SqlColumn): (AliasState, (Option[String], String)) = {
      if (ownerChain.exists(_.directlyOwns(column))) {
        column
          .namedOwner
          .map(named => tableRef(named).fmap(Option(_)))
          .getOrElse((this, None))
          .fmap(table0 => (table0, column.column))
      } else {
        val name = columnAliases
          .get((column.underlying.owner.context.resultPath, column.underlying.column))
          .getOrElse(column.column)
        column
          .namedOwner
          .map(named => tableRef(named).fmap(Option(_)))
          .getOrElse((this, None))
    ...
    /**
     * Update state to reflect the current column owner while traversing the `SqlQuery` being
     * rendered
     */
    def pushOwner(owner: ColumnOwner): (AliasState, Unit) =
      (copy(ownerChain = owner :: ownerChain), ())

    /**
     * Update state to restore the current column owner while traversing the `SqlQuery` being
     * rendered
     */
    def popOwner: (AliasState, ColumnOwner) =
      (copy(ownerChain = ownerChain.tail), ownerChain.head)
  }
   ...
   *
   * ColumnOwners are tables, SQL queries and subqueries, common table expressions and the like.
   * Most, but not all have a name (SqlSelect, SqlUnion and SqlJoin being unnamed examples)
   */
  ...
  trait SqlColumn {
     ...
     *
     * Used to represent columns on the outside of subqueries and common table expressions. Note
     * that column aliases are tracked across derivation so that derived columns will continue
     * to refer to the same underlying data irrespective of renaming.
     */
  ...
  object SqlColumn {
    def mkDefFragment(
        ...
        base: String,
        collated: Boolean,
        alias: String): Fragment = {
      val prefix0 = prefix.map(_ + ".").getOrElse("")
      val qualified = prefix0 + base
      val collated0 =
        if (collated)
          Fragments.const(s"($qualified") |+| collateToFragment |+| Fragments.const(")")
        else Fragments.const(qualified)

      if (base == alias) collated0
      else collated0 |+| Fragments.const(s" AS $alias")
    }

    def mkDefFragment(base: Fragment, collated: Boolean, alias: String): Fragment = {
      val collated0 =
        if (collated) Fragments.parentheses(base |+| collateToFragment)
        else base
     ...
     */
    case class TableColumn(owner: ColumnOwner, cr: ColumnRef, resultPath: List[String])
        extends SqlColumn {
    ...
    case class NullColumn(owner: ColumnOwner, col: SqlColumn) extends SqlColumn {
      ...
      def toDefFragment(collated: Boolean): Aliased[Fragment] =
        Aliased.columnDef(this).map {
          case (_, column0) =>
            mkDefFragment(ascribedNullToFragment(codec), collated, column0)
        }
    ...
    case class CountColumn(col: SqlColumn, cols: List[SqlColumn]) extends SqlColumn {
      ...
      def toDefFragment(collated: Boolean): Aliased[Fragment] =
        for {
          ...
          val count = Fragments.const("COUNT(DISTINCT(") |+| cols0.intercalate(
            Fragments.const(", ")) |+| Fragments.const(s"))")
     ...
     */
    case class PartitionColumn(
        owner: ColumnOwner,
        column: String,
        partitionCols: List[SqlColumn],
      ...
      def subst(from: ColumnOwner, to: ColumnOwner): SqlColumn =
        copy(
          owner = if (owner.isSameOwner(from)) to else owner,
          partitionCols = partitionCols.map(_.subst(from, to)))
      ...
      def toDefFragment(collated: Boolean): Aliased[Fragment] =
        for {
          ...
          val base =
            Fragments.const("dense_rank() OVER ") |+| Fragments.parentheses(cols0 |+| orderBy)
    ...
    case class FirstValueColumn(
      ...
      def toDefFragment(collated: Boolean): Aliased[Fragment] =
        for {
          col0 <- col.toRefFragment(false)
          cols0 <- partitionColsToFragment
          tc0 <- Aliased.columnDef(this)
          orderBy <- SqlQuery.ordersToFragment(orders)
        } yield {
          val base =
            Fragments.const("first_value") |+| Fragments.parentheses(col0) |+| Fragments.const(
              " OVER ") |+| Fragments.parentheses(cols0 |+| orderBy)
          mkDefFragment(base, false, tc0._2)
        }
     ...
     *
     * Columns of embedded subobjects have a different context path from columns of their
     * enclosing object, however they resolve to columns of the same `SqlSelect`. To satisfy the
     * `SqlSelect` invariant that all its columns must share the same context path we have to
     * wrap the embedded column so that its context path conforms.
     */
    ...
    case class DerivedColumn(owner: ColumnOwner, col: SqlColumn) extends SqlColumn {
      ...
      def toDefFragment(collated: Boolean): Aliased[Fragment] = {
        for {
          table0 <- namedOwner
            .map(named => Aliased.tableDef(named).map(Option(_)))
            .getOrElse(Aliased.pure(None))
  ...
  case class Join(conditions: List[(ColumnRef, ColumnRef)]) {
    ...
    def toSqlColumns(
        parentTable0: TableExpr,
        childTable0: TableExpr): List[(SqlColumn, SqlColumn)] = {
      parentCols(parentTable0).zip(childCols(childTable0))
    }

    def toSqlColumns(
        parentContext: Context,
        childContext: Context): List[(SqlColumn, SqlColumn)] =
   ...
   * work programmatically, but also because the operation isn't necessarily idempotent and the
   * result set doesn't necessarily contain the fields required for the filter predicates.
   */
  def stripCompiled(query: Query, context: Context): Result[Query] = {
    def loop(query: Query, context: Context): Query =
      query match {
        ...
        case FilterOrderByOffsetLimit(p @ Some(pred), oss, off, lim, child)
            if !isSqlTerm(context, pred).getOrElse(
              throw new SqlMappingException(s"Unmapped term $pred")) =>
        ...
        case other @ (_: Component[_] | _: Effect[_] | Empty | _: Introspect | _: Select |
            _: Count | _: UntypedFragmentSpread | _: UntypedInlineFragment) =>
          other
      }
  ...
  def sqlCursor(query: Query, env: Env): F[Result[Cursor]] =
    defaultRootCursor(
      query,
      schema.queryType,
      Some(RootCursor(Context(schema.queryType), None, env))).map(_.map(_._2))

  // Overrides definition in Mapping
  override def defaultRootCursor(
      query: Query,
      tpe: Type,
    ...
    def mkCursor(query: Query): F[Result[(Query, SqlCursor)]] =
      MappedQuery(query, context).flatTraverse { mapped =>
        (for {
          ...
          _ <- ResultT(
            monitor.queryMapped(query, frag, table.numRows, table.numCols).map(_.success))
  ...
  case class SqlField(
      ...
      associative: Boolean =
        false // a key which is also associative might occur multiple times in the table, ie. it is not a DB primary key
  ...
  /**
   * Common super type for mappings which have a programmatic discriminator, ie. interface and
   * union mappings.
   */
  ...
  object SqlInterfaceMapping {
    ...
    case class DefaultInterfaceMapping(
  ...
  object SqlUnionMapping {
    ...
    case class DefaultUnionMapping(
        predicate: MappingPredicate,
        fieldMappings: Seq[FieldMapping],
        discriminator: SqlDiscriminator)(
        implicit val pos: SourcePos
  ...
  def discriminatorColumnsForType(context: Context): List[SqlColumn] =
    typeMappings
      .objectMapping(context)
      .map(
        _.fieldMappings
          .iterator
          .collect {
            case cm: SqlField if cm.discriminator =>
              SqlColumn.TableColumn(context, cm.columnRef, cm.fieldName :: context.resultPath)
          }
          .toList)
      .getOrElse(Nil)
  ...
  def columnsForLeaf(context: Context, fieldName: String): Result[List[SqlColumn]] =
    typeMappings.fieldMapping(context, fieldName) match {
      case Some(SqlField(_, cr, _, _, _, _)) =>
        List(SqlColumn.TableColumn(context, cr, fieldName :: context.resultPath)).success
      case Some(SqlJson(_, cr)) =>
        List(SqlColumn.TableColumn(context, cr, fieldName :: context.resultPath)).success
  ...
  def columnForAtomicField(context: Context, fieldName: String): Result[SqlColumn] = {
    typeMappings.fieldMapping(context, fieldName) match {
      case Some(SqlField(_, cr, _, _, _, _)) =>
        SqlColumn.TableColumn(context, cr, fieldName :: context.resultPath).success
      case Some(SqlJson(_, cr)) =>
        SqlColumn.TableColumn(context, cr, fieldName :: context.resultPath).success
      case _ =>
        Result.internalError(s"No column for atomic field '$fieldName' in context $context")
    }
  }
  ...
  def encoderForTerm(context: Context, term: Term[_]): Result[Encoder] =
    term match {
      ...
      case _: And | _: Or | _: Not | _: Eql[_] | _: NEql[_] | _: Lt[_] | _: LtEql[_] |
          _: Gt[_] | _: GtEql[_] =>
        booleanEncoder.success
  ...
  def parentTableForType(context: Context): Result[TableRef] = {
    ...
    typeMappings.objectMapping(context).toResultOrError(noTable).flatMap { om =>
      om.fieldMappings
        .collectFirst { case SqlField(_, cr, _, _, _, _) => TableRef(context, cr.table) }
        .toResultOrError(noTable)
        .orElse {
          context.tpe.underlyingObject match {
            case Some(ot: ObjectType) =>
              ot.interfaces
                .collectFirstSome(nt => parentTableForType(context.asType(nt)).toOption)
                .toResultOrError(noTable)
  ...
  def isSqlTerm(context: Context, term: Term[_]): Result[Boolean] =
    term.forallR {
      ...
      case True | False | _: Const[_] | _: And | _: Or | _: Not | _: Eql[_] | _: NEql[_] |
          _: Contains[_] | _: Lt[_] | _: LtEql[_] | _: Gt[_] | _: GtEql[_] | _: In[_] |
          _: AndB | _: OrB | _: XorB | _: NotB | _: Matches | _: StartsWith | _: IsNull[_] |
          _: ToUpperCase | _: ToLowerCase | _: Like | _: SqlColumnTerm =>
        true.success
  ...
  sealed trait TableExpr extends ColumnOwner {
     ...
     *
     * This is used to determine whether or not non-nullable columns should be weakened to being
     * nullable when fetched
     */
    def isUnion: Boolean
  ...
  object TableExpr {
      ...
      def toDefFragment: Aliased[Fragment] =
        for {
          ...
          if (name == alias)
            ...
            Fragments.const(s"$name") |+| aliasDefToFragment(alias)
    ...
    object Laterality {
      def apply(lateral: Boolean, inner: Boolean): Laterality =
        if (lateral) mkLateral(inner) else NotLateral

      case object NotLateral extends Laterality {
    ...
    case class SubqueryRef(
      ...
      def joinToFragment(join: SqlJoin): Aliased[Fragment] =
        laterality.joinToFragment(join, this)

      override def delateral: TableExpr = copy(laterality = Laterality.NotLateral)
    }
    ...
    case class WithRef(context: Context, name: String, withQuery: SqlQuery) extends TableExpr {
      ...
      def contains(other: ColumnOwner): Boolean =
        isSameOwner(other) || withQuery.contains(other)
     ...
     *
     * Typically used where we need to refer to a table defined in a subquery or common table
     * expression.
     */
    case class DerivedTableRef(
        context: Context,
        alias: Option[String],
        underlying: TableExpr,
        noalias: Boolean = false)
        extends TableExpr {
      ...
      def contains(other: ColumnOwner): Boolean =
        isSameOwner(other) || underlying.contains(other)
      ...
      def toDefFragment: Aliased[Fragment] = {
        for {
          ...
          if (name == uname)
            ...
            Fragments.const(s"$uname") |+| aliasDefToFragment(name)
        }
      }
  ...
  sealed trait SqlQuery extends ColumnOwner {
    ...
    def withContext(
        ...
        extraJoins: List[SqlJoin]): Result[SqlQuery]
    ...
    /**
     * Yields a collection of `SqlSelects` which when combined as a union are equivalent to this
     * query
     */
  ...
  object SqlQuery {
    ...
    def combineAll(queries: List[SqlQuery]): Result[SqlQuery] = {
      if (queries.sizeCompare(1) <= 0)
        queries.headOption.toResultOrError("Expected at least one query in combineAll")
      else {
        val (selects, unions) =
        ...
        assert(
          allSelects.forall(sel =>
            sel.context == ctx && sel.limit.isEmpty && sel.offset.isEmpty && sel
              .orders
              .isEmpty && !sel.isDistinct))
        ...
        val combinedSelects =
          selects.groupBy(sel => sel.table).values.flatMap(combineCompatible).toList
      ...
      def contextualise(term: Term[_]): SqlColumn =
        contextualiseTerm(context, owner, term) match {
          ...
          case Result.Failure(_) =>
            throw new SqlMappingException(s"Failed to contextualise term $term")
    ...
    def whereToFragment(context: Context, pred: Predicate): Aliased[Fragment] = {
      def encoder1(enc: Option[Encoder], x: Term[_]): Encoder =
        enc.getOrElse(
          encoderForTerm(context, x).getOrElse(
            throw new SqlMappingException(s"No encoder for term $x")))

      def encoder2(enc: Option[Encoder], x: Term[_], y: Term[_]): Encoder =
        enc.getOrElse(
          encoderForTerm(context, x).getOrElse(encoderForTerm(context, y).getOrElse(
            throw new SqlMappingException(s"No encoder for terms $x or $y"))))
      ...
      def loop(term: Term[_], e: Encoder): Aliased[Fragment] = {
        ...
        def binaryOp(x: Term[_], y: Term[_])(
            op: Fragment,
            enc: Option[Encoder] = None): Aliased[Fragment] = {
        ...
        term match {
            ...
            binaryOp2(x)(
              fx =>
                Fragments.const("regexp_matches(") |+|
                  fx |+| Fragments.const(s", ") |+| Fragments
                    .bind(stringEncoder, regex.toString) |+|
                  Fragments.const(s")"),
              Some(stringEncoder)
            )
          ...
          case StartsWith(x, prefix) =>
            binaryOp2(x)(
              fx =>
                fx |+| Fragments.const(s" LIKE ") |+| Fragments
                  .bind(stringEncoder, prefix + "%"),
              Some(stringEncoder)
            )

          case ToUpperCase(x) =>
            binaryOp2(x)(
              fx => Fragments.const("upper(") |+| fx |+| Fragments.const(s")"),
              Some(stringEncoder)
            )

          case ToLowerCase(x) =>
            binaryOp2(x)(
              fx => Fragments.const("lower(") |+| fx |+| Fragments.const(s")"),
              Some(stringEncoder)
            )

          case IsNull(x, isNull) =>
            val sense = if (isNull) "" else "NOT"
            binaryOp2(x)(fx => fx |+| Fragments.const(s" IS $sense NULL "))

          case Like(x, pattern, caseInsensitive) =>
            binaryOp2(x)(
              fx => likeToFragment(fx, pattern, caseInsensitive),
              Some(stringEncoder)
            )
    ...
    def wheresToFragment(context: Context, wheres: List[Predicate]): Aliased[Fragment] =
      wheres
        .traverse(pred => whereToFragment(context, pred))
        .map(fwheres => Fragments.const(" ") |+| Fragments.whereAnd(fwheres: _*))
    ...
    def contextualiseOrderTerms[T](
      ...
      contextualiseTerm(context, owner, os.term).map { col =>
        os.subst(SqlColumnTerm(col).asInstanceOf[Term[T]])
      }

    def embedOrderTerms[T](
        table: TableExpr,
        parentTable: TableRef,
        os: OrderSelection[T]): Result[OrderSelection[T]] =
      embedTerm(table, parentTable, os.term).map { col =>
        os.subst(SqlColumnTerm(col).asInstanceOf[Term[T]])
      }

    /**
     * Yields a copy of the given `OrderSelection` with all occurences of `from` replaced by
     * `to`
     */
    def substOrderTables[T](
        from: TableExpr,
        to: TableExpr,
        os: OrderSelection[T]): OrderSelection[T] =
      os.term match {
              ...
              term match {
                case SqlColumnTerm(col) =>
                  for {
                    ...
                    orderToFragment(fc, ascending, nullsLast)
                  }
                case other =>
                  Aliased.internalError[Fragment](s"Unresolved term $other in ORDER BY")
              }
          }
          .map(forders =>
            Fragments.const(" ORDER BY ") |+| forders.intercalate(Fragments.const(",")))
    ...
    /**
     * Yield a copy of the given `Term` with all referenced `SqlColumns` relativised to the
     * given context and owned by by the given owner
     */
    def contextualiseTerm(
        context: Context,
        owner: ColumnOwner,
        term: Term[_]): Result[SqlColumn] = {
      ...
      term match {
        ...
        case pathTerm: PathTerm =>
          columnForSqlTerm(context, pathTerm).map { col =>
            val col0 = subst(col)
            if (isEmbeddedIn(col0.owner.context, owner.context))
              SqlColumn.EmbeddedColumn(owner, col0)
            else col0
          }
    ...
    case class EmptySqlQuery(context: Context) extends SqlQuery {
      ...
      def nest(
          ...
          extraCols: List[SqlColumn],
          oneToOne: Boolean,
          lateral: Boolean): Result[SqlQuery] =
      ...
      def toSubquery(name: String): Result[SqlSelect] =
        mkSelect.flatMap(_.toSubquery(name))

      def isUnion: Boolean = false
      def oneToOne: Boolean = false
      def toFragment: Aliased[Fragment] =
        Aliased.internalError("Attempt to render empty query as fragment")
    ...
    case class SqlSelect(
      ...
      private def owns0(col: SqlColumn): Boolean =
        isSameOwner(col.owner) || table.owns(col) || withs.exists(_.owns(col)) || joins.exists(
          _.owns(col))
      ...
      def withContext(
          context: Context,
          extraCols: List[SqlColumn],
          extraJoins: List[SqlJoin]): Result[SqlSelect] =
        copy(
          context = context,
          cols = (cols ++ extraCols).distinct,
          joins = extraJoins ++ joins).success

      def isUnion: Boolean =
        table.isUnion || context.tpe.underlying.isUnion || context.tpe.underlying.isInterface
      ...
      def contains(other: ColumnOwner): Boolean =
        isSameOwner(other) || table.contains(other) || joins.exists(_.contains(other)) || withs
          .exists(_.contains(other))
      ...
      def findNamedOwner(col: SqlColumn): Option[TableExpr] =
        table
          .findNamedOwner(col)
          .orElse(joins.collectFirstSome(_.findNamedOwner(col)))
          .orElse(withs.collectFirstSome(_.findNamedOwner(col)))
      ...
      lazy val orderCols: Result[List[SqlColumn]] =
        orders.traverse(os => columnForSqlTerm(context, os.term)).map(_.distinct)

      /**
       * Does the given column need collation?
       */
      def selectedNeedsCollation(col: SqlColumn): Result[Boolean] = {
        if (collateSelected && Fragments.needsCollation(col.codec))
          orderCols.map(_.contains(col))
        else false.success
      }

      /**
       * Yield a name for this select derived from any names associated with its from clauses or
       * joins
       */
          ...
          def mkJoins(joins: List[Join]): SqlSelect = {
            def mkSubquery(
              ...
              if (isMergeable) nested
                ...
                val base0 = nested
                  .copy(cols = (exposeCols ++ nested.cols).distinct)
                  .toSubquery(syntheticName(suffix), Laterality(lateral, inner))
            ...
            val initJoins = joins.init
            val multiJoinHead = initJoins.headOption
            val multiTable = multiJoinHead.nonEmpty
            val multiTableSuffixJoins =
              initJoins.drop(1).map(_.toSqlJoin(parentContext, parentContext, inner))
            val lastJoin = joins.last
            val lastJoinParentTable = lastJoin.parentTable(parentContext)
            ...
            val nestedJoins = multiTableSuffixJoins ++ finalJoins ++ base.joins
            val nested = mkNested(
              !multiTable,
              base.withs,
              nestedJoins.head.parent,
              base.cols,
              base.wheres,
              nestedJoins)

            multiJoinHead.fold(nested) { firstJoin0 =>
              val nestedBase =
                mkSubquery(false, nested, firstJoin0.childCols(nested.table), "_multi")
              val initialJoin = firstJoin0.toSqlJoin(parentTable, nestedBase.table, inner)
              mkNested(
                true,
                Nil,
                parentTable,
          ...
          val nested =
            typeMappings.fieldMapping(parentContext, fieldName) match {
              case Some(_: CursorFieldJson) | Some(SqlObject(_, Nil)) =>
                ...
                for {
                  embeddedWheres <- wheres.traverse(pred =>
                    embedWhereTerms(table, parentTable, pred))
                  embeddedOrders <- orders.traverse(os =>
                    embedOrderTerms(table, parentTable, os))
                } yield copy(
                  context = parentContext,
                  table = parentTable,
                  cols = embeddedCols,
              ...
              case _ =>
                Result.internalError(
                  s"Non-subobject mapping for field '$fieldName' of type ${parentContext.tpe}")
            }
      ...
      def addFilterOrderByOffsetLimit(
        ...
        def mkPredSubquery(base0: SqlSelect, predQuery: SqlSelect): SqlSelect = {
          ...
          val predSub = SubqueryRef(context, predName, predQuery, Laterality.NotLateral)
          ...
          base0.copy(
            ...
            wheres = base0.wheres ++ on.map { case (p, c) => Eql(p.toTerm, c.toTerm) },
            predicate = true
          )
        }
            ...
            if (oneToOne && predIsOneToOne && fastPathSafe) {
              ...
              pred0.traverse(p => contextualiseWhereTerms(context, table, p)).flatMap { pred1 =>
                oss.traverse(os => contextualiseOrderTerms(context, table, os)).flatMap {
                  oss0 =>
                    val orders = oss0 ++ keyCols
                      .diff(orderCols)
                      .map(col => OrderSelection(col.toTerm, true, nullsHigh))
                    val nonNullKeys = keyCols.map(col => IsNull(col.toTerm, false))

                    // We could use row_number in this case
                    val partitionCol =
                      SqlColumn.PartitionColumn(table, "row_item", partitionBy, orders)
                    val exposeCols = parentConstraints.lastOption.getOrElse(Nil).map {
                      case (_, col) =>
                        findNamedOwner(col)
                          .map(owner => col.derive(owner))
                          .getOrElse(col.derive(table))
                    }
                    ...
                    val subWithRowItem =
                      SubqueryRef(context, numberedName, selWithRowItem, Laterality.NotLateral)

                    val partitionTerm =
                      partitionCol.derive(subWithRowItem).toTerm.asInstanceOf[Term[Int]]
              ...
              pred0.traverse(p => contextualiseWhereTerms(context, baseRef, p)).flatMap {
                pred1 =>
                  oss.traverse(os => contextualiseOrderTerms(context, baseRef, os)).flatMap {
                    oss0 =>
                      val orders = oss0 ++ keyCols
                        .diff(orderCols)
                        .map(col => OrderSelection(col.derive(baseRef).toTerm, true, nullsHigh))
                      ...
                      val numberedName = syntheticName("_numbered")
                      val subWithRowItem = SubqueryRef(
                        context,
                        numberedName,
                        selWithRowItem,
              ...
              pred0.traverse(p => contextualiseWhereTerms(context, table, p)).flatMap { pred1 =>
                oss.traverse(os => contextualiseOrderTerms(context, table, os)).flatMap {
                  oss0 =>
                    oss0
                      .filterA(os => columnForSqlTerm(context, os.term).map(keyCols.contains))
                      .flatMap { nonKeyOrders =>
                        val orders = oss0 ++ keyCols
                          .diff(orderCols)
                          .map(col => OrderSelection(col.toTerm, true, nullsHigh))
                        val nonNullKeys = keyCols.map(col => IsNull(col.toTerm, false))

                        val distOrders = keyCols.map(col =>
                          OrderSelection(col.toTerm, true, nullsHigh)) ++ nonKeyOrders

                        val partitionCol =
                          SqlColumn.PartitionColumn(table, "row_item", partitionBy, orders)
                        val distPartitionCol = SqlColumn.PartitionColumn(
                          table,
                          "row_item_dist",
                          partitionBy ++ keyCols,
                          distOrders)
                        ...
                        val selWithRowItem =
                          SqlSelect(
                            ...
                            table = table,
                            cols =
                              (partitionCol :: distPartitionCol :: exposeCols ++ cols ++ orderCols).distinct,
                            joins = joins,
                            wheres = (pred1 ++ nonNullKeys ++ wheres).distinct,
                            orders = Nil,
                            offset = None,
                        ...
                        val partitionTerm =
                          partitionCol.derive(subWithRowItem).toTerm.asInstanceOf[Term[Int]]
                        val distPartitionTerm =
                          distPartitionCol.derive(subWithRowItem).toTerm.asInstanceOf[Term[Int]]
                        val windowPred =
                          And(mkWindowPred(partitionTerm), LtEql(distPartitionTerm, Const(1)))
                      ...
                      oss0
                        ...
                        .flatMap { nonKeyOrders =>
                          val orders = oss0 ++ keyCols
                            .diff(orderCols)
                            .map(col =>
                              OrderSelection(col.derive(baseRef).toTerm, true, nullsHigh))
                          val predCols = keyCols.map(_.derive(baseRef))
                          val nonNullKeys = predCols.map(col => IsNull(col.toTerm, false))

                          val distOrders = keyCols.map(col =>
                            OrderSelection(
                              col.derive(baseRef).toTerm,
                              true,
                              nullsHigh)) ++ nonKeyOrders
                          ...
                          val partitionTerm =
                            partitionCol.derive(subWithRowItem).toTerm.asInstanceOf[Term[Int]]
                          val distPartitionTerm = distPartitionCol
                            .derive(subWithRowItem)
                            .toTerm
                            .asInstanceOf[Term[Int]]
                          val windowPred =
                            And(mkWindowPred(partitionTerm), LtEql(distPartitionTerm, Const(1)))
                  ...
                  pred1 =>
                    oss.traverse(os => contextualiseOrderTerms(context, baseRef, os)).flatMap {
                      oss0 =>
                        oss0
                          .filterA(os =>
                            columnForSqlTerm(context, os.term).map(keyCols.contains))
                          .flatMap { nonKeyOrders =>
                            val distOrders = keyCols.map(col =>
                              OrderSelection(
                                col.derive(baseRef).toTerm,
                            ...
                            val distName = "dist"
                            val distSub =
                              SubqueryRef(context, distName, distQuery, Laterality.NotLateral)

                            val predCols0 = keyCols.map(_.derive(distSub))
                            oss
                              .traverse(os => contextualiseOrderTerms(context, distSub, os))
                              .flatMap { outerOss0 =>
                                val orders = outerOss0 ++ keyCols
                                  .diff(orderCols)
                                  .map(col =>
                                    OrderSelection(col.derive(distSub).toTerm, true, nullsHigh))
                                val predQuery = SqlSelect(
                                  context = context,
                                  withs = Nil,
                                  table = distSub,
      ...
      def toFragment: Aliased[Fragment] = {
        for {
          ...
          withs0 <-
            if (withs.isEmpty) Aliased.pure(Fragments.empty)
            else
              withs
                .traverse(_.toDefFragment)
                .map(fwiths =>
                  Fragments.const("WITH ") |+| fwiths.intercalate(Fragments.const(",")))
          table0 <- table.delateral.toDefFragment
          cols0 <- cols.traverse(col =>
            Aliased.liftR(selectedNeedsCollation(col)).flatMap(col.toDefFragment))
          dcols <- distinct.traverse(col =>
            Aliased.liftR(selectedNeedsCollation(col)).flatMap(col.toRefFragment))
          dist = if (dcols.isEmpty) Fragments.empty else distinctOnToFragment(dcols)
          joins0 <- joins.traverse(_.toFragment).map(_.combineAll)
          select = Fragments.const(s"SELECT ") |+| dist |+| cols0.intercalate(
            Fragments.const(", "))
          ...
          off = offset
            .orElse(defaultOffsetForLimit(limit))
            .map(o => offsetToFragment(Fragments.const(s"$o")))
            .getOrElse(Fragments.empty)
          lim = limit
            .map(l => limitToFragment(Fragments.const(s"$l")))
            .getOrElse(Fragments.empty)
          _ <- Aliased.popOwner
        } yield withs0 |+| select |+| from |+| joins0 |+| where |+| orderBy |+| off |+| lim
      }
    }
    ...
    case class SqlUnion(elems: List[SqlSelect]) extends SqlQuery {
      ...
      if (topLevel)
        assert(
          elems
            .tail
            .forall(elem =>
              elem.context.path.sizeCompare(1) == 0 || schema.isRootType(elem.context.tpe)))
      else
      ...
      def withContext(
          context: Context,
          extraCols: List[SqlColumn],
          extraJoins: List[SqlJoin]): Result[SqlUnion] =
      ...
      def contains(other: ColumnOwner): Boolean =
        isSameOwner(other) || elems.exists(_.contains(other))
      ...
      /**
       * The union of the columns of the underlying SELECTs in the order they will be yielded as
       * the columns of this UNION
       */
      ...
      def toSubquery(name: String): Result[SqlSelect] = {
        val sub = SubqueryRef(context, name, this, Laterality.NotLateral)
        SqlSelect(
          context,
          Nil,
          sub,
          cols.map(_.derive(sub)),
          Nil,
          Nil,
          Nil,
          None,
        ...
        val withFilter =
          (filter, limit) match {
            ...
            case _ =>
              ...
              val elems0 =
                elems.foldLeft(List.empty[SqlSelect].success) {
                  case (elems0, elem) =>
                    elems0.flatMap { elems0 =>
                      elem
                        .addFilterOrderByOffsetLimit(
                          filter,
                          branchOrderBy,
                          None,
                          branchLimit,
    ...
    object SqlUnion {
      def apply(elems: List[SqlSelect]): SqlUnion =
        new SqlUnion(elems.distinct.map(encapsulateUnionBranch))
    }
    ...
    case class SqlJoin(
      ...
      def subst(from: TableExpr, to: TableExpr): SqlJoin = {
        ...
        val newChild =
          if (!child.isSameOwner(from)) child
            ...
            (child, to) match {
              case (sr: SubqueryRef, to: TableRef) =>
                sr.copy(context = to.context, name = to.name)
              case _ => to
            }
          }
      ...
      /**
       * Return the columns of `table` referred to by the parent side of the conditions of this
       * join
       */
      ...
      def toFragment: Aliased[Fragment] = {
        child match {
          case sq: SubqueryRef =>
            sq.joinToFragment(this)

          case _ =>
            toFragmentWithoutLaterality
        }
      }

      def toFragmentWithoutLaterality: Aliased[Fragment] = {
  ...
  sealed trait MappedQuery {
    ...
    /**
     * Yield a list of `Tables` one for each of the subobjects of the context type contained in
     * `table`.
     */
  ...
  object MappedQuery {
    ...
    def apply(q: Query, context: Context): Result[MappedQuery] = {
      def loop(
          ...
          context: Context,
          parentConstraints: List[List[(SqlColumn, SqlColumn)]],
          exposeJoins: Boolean): Result[SqlQuery] = {
        ...
        def group(queries: List[Query]): Result[SqlQuery] = {
          queries
            .foldLeft(List.empty[SqlQuery].success) {
              case (nodes, q) =>
        ...
        def unembed(context: Context): Context = {
          if (context.path.sizeCompare(1) <= 0) context
            ...
            val parentContext = context.copy(
              path = context.path.tail,
              resultPath = context.resultPath.tail,
              typePath = context.typePath.tail)
            val fieldName = context.path.head
        ...
        def parentConstraintsFromJoins(
            parentContext: Context,
            fieldName: String,
            resultName: String): Result[List[List[(SqlColumn, SqlColumn)]]] = {
        ...
        def parentConstraintsToSqlJoins(
            parentTable: TableRef,
            parentConstraints: List[List[(SqlColumn, SqlColumn)]]): Result[List[SqlJoin]] =
        ...
        q match {
            ...
            for {
              ...
              res <-
                sq match {
                  case sq: SqlSelect =>
                    sq.joins match {
                      case hd :: tl =>
                        ...
                        val ssq = sq.copy(
                          table = hd.child,
                          cols = SqlColumn.CountColumn(
                            countCol.in(hd.child),
                            keyCols.map(_.in(hd.child))) :: Nil,
                          joins = tl,
                          wheres = wheres)
                      ...
                      case _ =>
                        ...
                        val ssq = sq.copy(cols = SqlColumn.CountColumn(
                          countCol.in(countTable),
                          keyCols.map(_.in(countTable))) :: Nil)
                        val ssqCol = SqlColumn.SubqueryColumn(countCol, ssq)
                        SqlSelect(
                          context,
                          Nil,
                  ...
                  case _ =>
                    Result.internalError(
                      "Implementation restriction: cannot count an SQL union")
                }
            } yield res
          ...
          case NonSubobjectSelect(fieldName) =>
            columnsForLeaf(context, fieldName).flatMap { cols =>
              val constraintCols =
                if (exposeJoins) parentConstraints.lastOption.getOrElse(Nil).map(_._2) else Nil
          ...
          case s @ Select(fieldName, resultName, child) =>
            context.forField(fieldName, resultName).flatMap { fieldContext =>
              if (schema.isRootType(context.tpe)) loop(child, fieldContext, Nil, false)
                ...
                val keyCols = keyColumnsForType(context)
                val constraintCols =
                  if (exposeJoins) parentConstraints.lastOption.getOrElse(Nil).map(_._2)
                  else Nil
                val extraCols = keyCols ++ constraintCols
                for {
                  parentTable <- parentTableForType(context)
                  parentConstraints0 <- parentConstraintsFromJoins(
                    context,
                    fieldName,
                    s.resultName)
                  extraJoins <- parentConstraintsToSqlJoins(parentTable, parentConstraints)
                  res <-
                    loop(child, fieldContext, parentConstraints0, false).flatMap { sq =>
                      if (parentTable.isRoot) {
                        assert(
                          parentConstraints0.isEmpty && extraCols.isEmpty && extraJoins.isEmpty)
                        sq.withContext(context, Nil, Nil)
                      } else
                        sq.nest(
                          context,
                          extraCols,
                          sq.oneToOne && isSingular(context, fieldName, child),
                          parentConstraints0.nonEmpty)
                          .flatMap { sq0 => sq0.withContext(sq0.context, Nil, extraJoins) }
                    }
                } yield res
              }
            }
          ...
          case TypeCase(default, narrows0) =>
            ...
            assert(
              supertpe.underlying.isInterface || supertpe.underlying.isUnion || (subtpes
                .sizeCompare(1) == 0 && subtpes.head =:= supertpe))
            ...
            val extraCols =
              (keyColumnsForType(context) ++ discriminatorColumnsForType(context)).distinct

            if (allSimple) {
              for {
                dquery <- loop(default, context, parentConstraints, exposeJoins).flatMap(
                  _.withContext(context, extraCols, Nil))
                nqueries <-
                  narrows.traverse { narrow =>
                    val subtpe0 = narrow.subtpe.withModifiersOf(context.tpe)
                    loop(narrow.child, context.asType(subtpe0), parentConstraints, exposeJoins)
                      .flatMap(_.withContext(context, extraCols, Nil))
                  }
              ...
              val nqueries =
                narrows.traverse { narrow =>
                  ...
                  loop(child, context.asType(subtpe0), parentConstraints, exposeJoins)
                    .flatMap(_.withContext(context, extraCols, Nil))
                }
              ...
              val dquery =
                if (exhaustive) EmptySqlQuery(context).success
                  ...
                  discriminatorForType(context)
                    .map { disc => subtpes.traverse(disc.discriminator.narrowPredicate) }
                    .getOrElse(Nil.success)
                    .flatMap { allPreds =>
                      if (exclusive) {
                        for {
                          parentTable <- parentTableForType(context)
                          allPreds0 <- allPreds.traverse(pred =>
                            SqlQuery
                              .contextualiseWhereTerms(context, parentTable, pred)
                              .map(Not(_)))
                        } yield {
                        ...
                        val defaultPredicate = And.combineAll(allPreds0)
                        loop(
                          Filter(defaultPredicate, default),
                          context,
                          parentConstraints,
                          exposeJoins).flatMap(_.withContext(context, extraCols, Nil))
                      }
                    }
          ...
          case Unique(child) =>
            loop(
              child,
              context.asType(context.tpe.nonNull.list),
              parentConstraints,
              exposeJoins).flatMap(_.withContext(context, Nil, Nil))
          ...
          case FilterOrderByOffsetLimit(pred, oss, offset, lim, child) =>
            ...
            val orderPaths = oss
              .map(_.map(_.term).collect { case path: PathTerm => path.path })
              .getOrElse(Nil)
              .distinct
            ...
            if (pred.exists(p => !isSqlTerm(context, p).getOrElse(false))) {
              ...
              val filter0 =
                (for {
                  ...
                  sq <- OptionT(
                    loop(filterQuery, context, parentConstraints, exposeJoins).map(_.some))
              ...
              val orderBy0 =
                (for {
                  ...
                  sq <- OptionT(
                    loop(orderQuery, context, parentConstraints, exposeJoins).map(_.some))
              ...
              for {
                ...
                res <- expandedChild.addFilterOrderByOffsetLimit(
                  filter.map(_._1),
                  orderBy.map(_._1),
                  offset,
                  lim,
          ...
          case Effect(_, _) =>
            Result.internalError("Effect node must be a child of a Select node")

          case Empty | Query.Component(_, _, _) | _: UntypedSelect | _: UntypedFragmentSpread |
              _: UntypedInlineFragment | _: Select =>
    ...
    final class NonEmptyMappedQuery(query: SqlQuery) extends MappedQuery {
      ...
      val colsByResultPath: Map[List[String], List[(SqlColumn, Int)]] =
        query
          .cols
          .filter(_.resultPath.nonEmpty)
          .groupMap(_.resultPath)(col => (col, index(col)))
      ...
      /**
       * Yield a list of `Tables` one for each of the subobjects of the context type contained
       * in `table`.
       */
      def group(context: Context, table: Table): Iterator[Table] =
      ...
      def leafIndex(context: Context, fieldName: String): Int =
        colsByResultPath.get(fieldName :: context.resultPath) match {
          ...
          case Some(cols) =>
            columnForAtomicField(context, fieldName) match {
              case Result.Success(cursorCol) =>
                cols.find(_._1 == cursorCol).map(_._2).getOrElse(-1)
              case Result.Warning(_, cursorCol) =>
                cols.find(_._1 == cursorCol).map(_._2).getOrElse(-1)
              case _ => -1
            }
        }
  ...
  sealed trait Table {
    ...
    /**
     * A copy of this `Table` containing only the rows for which all the given columns are
     * defined
     */
  ...
  case class SqlCursor(
      ...
      extends Cursor {
    assert(
      focus != Table.EmptyTable || context.tpe.isNullable || context.tpe.isList || schema
        .isRootType(context.tpe) || parentTableForType(context).map(_.isRoot).getOrElse(false))
      ...
      tpe
        ...
        .map(itemTpe =>
          asTable.map { table =>
            ...
            mapped
              .group(itemContext, table)
              .map(table => mkChild(itemContext, focus = table))
              .to(factory)
          })
    ...
    def asNullable: Result[Option[Cursor]] =
      (tpe, focus) match {
        ...
        case (NullableType(tpe), _) =>
          Some(
            mkChild(
              context.asType(tpe))).success // non-nullable column as nullable schema type (ok)
    ...
    def narrowsTo(subtpe: TypeRef): Result[Boolean] = {
      def check(ctpe: Type): Boolean =
        if (ctpe =:= tpe)
          asTable
            .map(table => mapped.narrowsTo(context.asType(subtpe), table))
            .toOption
            .getOrElse(false)
        else ctpe <:< subtpe
        ...
        typeMappings.fieldMapping(this, fieldName).flatMap {
          case Some((np, _: SqlJson)) =>
            ...
            asTable.flatMap { table =>
              ...
              mapped
                .selectAtomicField(context, fieldName, table)
                .flatMap(_ match {
                  ...
                  case other =>
                    Result.internalError(
                      s"$fieldTpe: expected jsonb value found ${other.getClass}: $other")
                })
            }
  ...
  case class MultiRootCursor(roots: List[SqlCursor]) extends Cursor.AbstractCursor {
    ...
    override def field(fieldName: String, resultName: Option[String]): Result[Cursor] = {
      roots
        .find(_.mapped.containsRoot(fieldName, resultName))
        .map(_.field(fieldName, resultName))
   ...
   */
  override protected def validateTypeMapping(
      mappings: TypeMappings,
      context: Context,
      tm: TypeMapping): List[ValidationFailure] = {
    ...
    def checkKey(om: ObjectMapping): List[ValidationFailure] =
      if (keyColumnsForType(context).nonEmpty || parentTableForType(context)
          .map(_.isRoot)
          .getOrElse(false)) Nil
    ...
    def checkDiscriminator(om: ObjectMapping): List[ValidationFailure] = {
      val dnmes =
        om.fieldMappings.collect { case sf: SqlField if sf.discriminator => sf.fieldName }
      dnmes.collectFirstSome { dnme =>
        typeMappings.rawFieldMapping(context, dnme) match {
          case Some(pf: TypeMappings.PolymorphicFieldMapping) =>
            Some((pf.candidates.head._2, pf.candidates.map(_._1.tpe.name)))
          case _ => None
        }
      } match {
        case None => Nil
    ...
    def checkSuperInterfaces(om: ObjectMapping): List[ValidationFailure] = {
      val allMappings = om.tpe.dealias match {
        case twf: TypeWithFields =>
          om :: twf.interfaces.flatMap(nt => mappings.objectMapping(context.asType(nt)))
        case _ => Nil
      }
   ...
   */
  override protected def validateFieldMapping(
      mappings: TypeMappings,
      context: Context,
      om: ObjectMapping,
      fm: FieldMapping): List[ValidationFailure] = {
    ...
    (fm, tpe.fieldInfo(fm.fieldName)) match {
      case (sf: SqlField, Some(field)) =>
        ...
        val leafMapping0 =
          mappings.typeMapping(fieldContext).collectFirst { case lm: LeafMapping[_] => lm }
        (field.tpe.dealias.nonNull, leafMapping0) match {
          case (_: ScalarType | _: EnumType, Some(leafMapping)) =>
            if (colIsNullable != fieldIsNullable)
              List(
                InconsistentlyNullableFieldMapping(om, sf, field, sf.columnRef, colIsNullable))
            else
                ...
                val serConsistent = {
                  ...
                  nonEmptyJoins match {
                    ...
                    case hd :: tl =>
                      ...
                      val lastIsChild =
                        nonEmptyJoins.last.conditions.head._2.table == childTable
                      val consistentChain =
                        nonEmptyJoins.zip(tl).forall {
                          case (j0, j1) =>
                            j0.conditions.head._2.table == j1.conditions.head._1.table
                        }
  ...
  case class NoJoinConditions(objectMapping: ObjectMapping, fieldMapping: FieldMapping)
    ...
    override def formattedMessage: String =
      s"""|No join conditions in field mapping.
          |
          |- The ${scala(objectMapping.showMappingType)} for type ${graphql(
           showNamedType(objectMapping.tpe))} at (1) has a ${scala(
           "SqlObject")} field mapping for the field ${graphql(
           fieldMapping.fieldName)} at (2) with a ${scala("Join")} with no join conditions.
   ...
   */
  case class InconsistentJoinConditions(
    ...
    override def formattedMessage: String =
      s"""|Inconsistent join conditions in field mapping.
          ...
          |- The ${scala(objectMapping.showMappingType)} for type ${graphql(
           showNamedType(objectMapping.tpe))} at (1) has a ${scala(
           "SqlObject")} field mapping for the field ${graphql(
           fieldMapping.fieldName)} at (2) with a Join with inconsistent join conditions: ${sql(
           s"(${parents.mkString(", ")}) -> (${children.mkString(", ")})")}.
    ...
    override def formattedMessage: String =
      s"""|Misaligned joins in field mapping.
          ...
          |- The ${scala(objectMapping.showMappingType)} for type ${graphql(
           showNamedType(objectMapping.tpe))} at (1) has a ${scala(
           "SqlObject")} field mapping for the field ${graphql(
           fieldMapping.fieldName)} at (2) with misaligned joins: ${sql(
           s"$parent, $child, ${path.mkString(", ")}")}.
  ...
  case class NoKeyInObjectTypeMapping(objectMapping: ObjectMapping)
    ...
    override def formattedMessage: String =
      s"""|No key field mapping in object type mapping.
          ...
          |- The ${scala(objectMapping.showMappingType)} for type ${graphql(
           showNamedType(
             objectMapping.tpe))} at (1) has no direct or inherited key field mapping.
  ...
  case class SplitObjectTypeMapping(objectMapping: ObjectMapping, tables: List[String])
    ...
    override def formattedMessage: String =
      s"""|Object type mapping is split across multiple tables.
          ...
          |- The ${scala(objectMapping.showMappingType)} for type ${graphql(
           showNamedType(
             objectMapping.tpe))} defined at (1) is split across multiple tables: ${sql(
           s"${tables.mkString(", ")}")}.
   ...
   */
  case class SplitEmbeddedObjectTypeMapping(
      parent: ObjectMapping,
      parentField: FieldMapping,
      child: ObjectMapping,
      parentTables: List[String],
      childTables: List[String])
      extends SqlValidationFailure(Severity.Error) {
    override def toString: String =
    ...
    override def formattedMessage: String =
      s"""|Embedded object type maps to non-parent tables.
          |
          |- The ${scala(parent.showMappingType)} for type ${graphql(
           showNamedType(parent.tpe))} defined at (1) embeds the ${scala(
           child.showMappingType)} for type ${graphql(
           showNamedType(child.tpe))} defined at (2) via field mapping ${graphql(
           s"${showNamedType(parent.tpe)}.${parentField.fieldName}")} at (3).
   ...
   */
  case class SplitInterfaceTypeMapping(
      objectMapping: ObjectMapping,
      intrfs: List[ObjectMapping],
      tables: List[String])
      extends SqlValidationFailure(Severity.Error) {
    override def toString: String =
    ...
    override def formattedMessage: String =
      s"""|Interface implementors are split across multiple tables.
          |
          |- The ${scala(objectMapping.showMappingType)} for type ${graphql(
           showNamedType(objectMapping.tpe))} at (1) has implementors (${intrfs
           .map(_.tpe.name)
           .mkString(", ")}) which are split across multiple tables: ${sql(
           s"${tables.mkString(", ")}")}.
   ...
   * Interface/union implementation mappings split across multiple tables
   */
  case class SplitUnionTypeMapping(
      objectMapping: ObjectMapping,
      members: List[ObjectMapping],
      tables: List[String])
      extends SqlValidationFailure(Severity.Error) {
    override def toString: String =
    ...
    override def formattedMessage: String =
      s"""|Union member mappings are split across multiple tables.
          |
          |- The ${scala(objectMapping.showMappingType)} for type ${graphql(
           showNamedType(objectMapping.tpe))} at (1) has members (${members
           .map(_.tpe.name)
           .mkString(", ")}) which are split across multiple tables: ${sql(
           s"${tables.mkString(", ")}")}.
  ...
  case class NoDiscriminatorInObjectTypeMapping(objectMapping: ObjectMapping)
    ...
    override def formattedMessage: String =
      s"""|No discriminator field mapping in interface/union type mapping.
          |
          |- The ${scala(objectMapping.showMappingType)} for type ${graphql(
           showNamedType(objectMapping.tpe))} at (1) has no discriminator field mapping.
   ...
   */
  case class IllegalPolymorphicDiscriminatorFieldMapping(
      objectMapping: ObjectMapping,
      fieldMapping: FieldMapping,
      impls: List[String])
      extends SqlValidationFailure(Severity.Error) {
    ...
    override def formattedMessage: String =
      s"""|Illegal polymorphic discriminator field mapping.
          ...
          |- The ${scala(objectMapping.showMappingType)} for type ${graphql(
           showNamedType(
             objectMapping.tpe))} at (1) contains a discriminator field mapping ${graphql(
           fieldMapping.fieldName)} at (2).
          |- This discriminator has variant field mappings in the type mappings for subtypes: ${impls
           .mkString(", ")}.
   ...
   */
  case class IllegalSubobjectInUnionTypeMapping(
      objectMapping: ObjectMapping,
      fieldMapping: FieldMapping)
      extends SqlValidationFailure(Severity.Error) {
    override def toString: String =
    ...
    override def formattedMessage: String =
      s"""|Illegal subobject field mapping in union type mapping.
          ...
          |- The ${scala(objectMapping.showMappingType)} for type ${graphql(
           showNamedType(
             objectMapping.tpe))} at (1) contains a subobject field mapping ${graphql(
           fieldMapping.fieldName)} at (2).
   ...
   */
  case class IllegalJsonInUnionTypeMapping(
      objectMapping: ObjectMapping,
      fieldMapping: FieldMapping)
      extends SqlValidationFailure(Severity.Error) {
    override def toString: String =
  ...
  case class AssocFieldNotKey(objectMapping: ObjectMapping, fieldMapping: FieldMapping)
    ...
    override def formattedMessage: String =
      s"""|Non-key associatitve field mapping in object type mapping.
          ...
          |- The ${scala(objectMapping.showMappingType)} for type ${graphql(
           showNamedType(
             objectMapping.tpe))} at (1) contains an associative field mapping ${graphql(
           fieldMapping.fieldName)} at (2) which is not a key.
   ...
   * Union field mappings must be hidden
   */
  case class NonHiddenUnionFieldMapping(
      objectMapping: ObjectMapping,
      fieldMapping: FieldMapping)
      extends SqlValidationFailure(Severity.Error) {
    override def toString: String =
    ...
    override def formattedMessage: String =
      s"""|Non-hidden field mapping in union type mapping.
          |
          |- The ${scala(objectMapping.showMappingType)} for type ${graphql(
           showNamedType(objectMapping.tpe))} at (1) contains a field mapping ${graphql(
           fieldMapping.fieldName)} at (2) which is not hidden.
   ...
   */
  case class InconsistentFieldLeafMapping(
      objectMapping: ObjectMapping,
      fieldMapping: FieldMapping,
      field: Field,
      columnRef: ColumnRef,
      leafMapping: LeafMapping[_])
      extends SqlValidationFailure(Severity.Error) {
    ...
    override def formattedMessage: String = {
      s"""|Inconsistent field leaf mapping.
          |
          |- The field ${graphql(
           s"${showNamedType(objectMapping.tpe)}.${fieldMapping.fieldName}: ${showType(field.tpe)}")} is defined by a Schema at (1).
          |- The ${scala(fieldMapping.showMappingType)} at (2) and ColumnRef for ${sql(
           s"${columnRef.table}.${columnRef.column}")} at (3) map ${graphql(
           showNamedType(leafMapping.tpe))} to Scala type ${scala(columnRef.scalaTypeName)}.
          |- A ${scala(leafMapping.showMappingType)} at (4) maps ${graphql(
           showNamedType(leafMapping.tpe))} to Scala type ${scala(leafMapping.scalaTypeName)}.
   ...
   * SqlField codec and LeafMapping are inconsistent.
   */
  case class InconsistentFieldTypeMapping(
      objectMapping: ObjectMapping,
      fieldMapping: FieldMapping,
      field: Field,
      columnRef: ColumnRef,
    ...
    override def formattedMessage: String = {
      s"""|Inconsistent field type mapping.
          ...
          |- The field ${graphql(
           s"${showNamedType(objectMapping.tpe)}.${fieldMapping.fieldName}: ${showType(field.tpe)}")} is defined by a Schema at (1).
          |- The ${scala(fieldMapping.showMappingType)} at (2) and ColumnRef for ${sql(
           s"${columnRef.table}.${columnRef.column}")} at (3) map to Scala type ${scala(
   ...
   * SqlField codec and LeafMapping are inconsistent.
   */
  case class InconsistentlyNullableFieldMapping(
      objectMapping: ObjectMapping,
      fieldMapping: FieldMapping,
      field: Field,
      columnRef: ColumnRef,
    ...
    override def formattedMessage: String = {
      ...
      s"""|Inconsistently nullable field mapping.
          ...
          |- The corresponding ColumnRef for ${sql(
           s"${columnRef.table}.${columnRef.column}")} at (4) is ${colNullability}.
