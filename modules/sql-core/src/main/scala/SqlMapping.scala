...
trait SqlMappingLike[F[_]] extends CirceMappingLike[F] with SqlModule[F] { self =>
  ...
  object SqlColumn {
    def mkDefFragment(prefix: Option[String], base: String, collated: Boolean, alias: String): Fragment = {
      ...
      val collated0 =
        if (collated) Fragments.const(s"($qualified") |+| collateToFragment |+| Fragments.const(")")
    ...
    case class FirstValueColumn(owner: ColumnOwner, col: SqlColumn, partitionCols: List[SqlColumn], orders: List[OrderSelection[_]]) extends SqlColumn {
      def column: String = col.column
      def codec: Codec = col.codec
      def scalaTypeName: String = col.scalaTypeName
      def pos: SourcePos = col.pos
      ...
      def subst(from: ColumnOwner, to: ColumnOwner): SqlColumn =
        copy(owner = if(owner.isSameOwner(from)) to else owner, col = col.subst(from, to), partitionCols = partitionCols.map(_.subst(from, to)))
      ...
      def toDefFragment(collated: Boolean): Aliased[Fragment] =
        for {
          ...
          val base = Fragments.const("first_value") |+| Fragments.parentheses(col0) |+| Fragments.const(" OVER ") |+| Fragments.parentheses(cols0 |+| orderBy)
          mkDefFragment(base, false, tc0._2)
        }
  ...
  object TableExpr {
    ...
    object Laterality {
      ...
      case object NotLateral extends Laterality {
        ...
        def joinToFragment(join: SqlJoin, subquery: SubqueryRef): Aliased[Fragment] = join.toFragmentWithoutLaterality
      ...
      case class Apply(inner: Boolean) extends Laterality {
        def toFragment: Fragment = Fragments.const(if(inner) "CROSS " else "OUTER ") |+| Fragments.const("APPLY ")
        def joinToFragment(join: SqlJoin, subquery: SubqueryRef): Aliased[Fragment] = subquery.toDefFragment
        def joinPredicates(join: SqlJoin): List[Predicate] =
          if(!join.isPredicate) join.on.map { case (p, c) => Eql(p.toTerm, c.toTerm) }
          else Nil
      }
    }

    /** Table expression corresponding to a subquery */
    case class SubqueryRef(context: Context, name: String, subquery: SqlQuery, laterality: Laterality) extends TableExpr {
      ...
      def toDefFragment: Aliased[Fragment] =
        ...
        } yield laterality.toFragment |+| Fragments.parentheses(sub) |+| aliasDefToFragment(alias)
  ...
  object SqlQuery {
      ...
      def loop(term: Term[_], e: Encoder): Aliased[Fragment] = {
        ...
        term match {
          ...
          case Like(x, pattern, caseInsensitive) =>
            binaryOp2(x)(
              fx =>
                likeToFragment(fx, pattern, caseInsensitive),
              Some(stringEncoder)
            )
    ...
    case class SqlSelect(
      ...
      def isUnion: Boolean = table.isUnion || context.tpe.underlying.isUnion || context.tpe.underlying.isInterface
      ...
      def selectedNeedsCollation(col: SqlColumn): Result[Boolean] = {
        if(collateSelected && Fragments.needsCollation(col.codec)) orderCols.map(_.contains(col))
        else false.success
      }
          ...
          def mkJoins(joins: List[Join]): SqlSelect = {
            def mkSubquery(multiTable: Boolean, nested: SqlSelect, joinCols: List[SqlColumn], suffix: String): SqlSelect = {
              ...
              if(isMergeable) nested
                ...
                val base0 = nested.copy(cols = (exposeCols ++ nested.cols).distinct).toSubquery(syntheticName(suffix), Laterality(lateral, inner))
                base0.copy(cols = nested.cols.map(_.derive(base0.table)))
              }
            }

            def mkNested(outer: Boolean, withs: List[WithRef], table: TableExpr, cols: List[SqlColumn], wheres: List[Predicate], joins: List[SqlJoin]): SqlSelect = {
              val extraWheres =
                if(!outer) Nil
                else
                  joins.flatMap { join =>
            ...
            val multiTableSuffixJoins = initJoins.drop(1).map(_.toSqlJoin(parentContext, parentContext, inner))
            ...
            val nested = mkNested(!multiTable, base.withs, nestedJoins.head.parent, base.cols, base.wheres, nestedJoins)

            multiJoinHead.fold(nested) { firstJoin0 =>
              val nestedBase = mkSubquery(false, nested, firstJoin0.childCols(nested.table), "_multi")
              val initialJoin = firstJoin0.toSqlJoin(parentTable, nestedBase.table, inner)
              mkNested(true, Nil, parentTable, nestedBase.cols, Nil, initialJoin :: nestedBase.joins)
            }
          }
            ...
            if(oneToOne && predIsOneToOne) {
              ...
              pred0.traverse(p => contextualiseWhereTerms(context, table, p)).flatMap { pred1 =>
                oss.traverse(os => contextualiseOrderTerms(context, table, os)).flatMap { oss0 =>
                  ...
                  val orders = oss0 ++ keyCols.diff(orderCols).map(col => OrderSelection(col.toTerm, true, nullsHigh))
                  ...
                  val subWithRowItem = SubqueryRef(context, numberedName, selWithRowItem, Laterality.NotLateral)
              ...
              pred0.traverse(p => contextualiseWhereTerms(context, baseRef, p)).flatMap { pred1 =>
                oss.traverse(os => contextualiseOrderTerms(context, baseRef, os)).flatMap { oss0 =>
                  ...
                  val orders = oss0 ++ keyCols.diff(orderCols).map(col => OrderSelection(col.derive(baseRef).toTerm, true, nullsHigh))
              ...
              pred0.traverse(p => contextualiseWhereTerms(context, table, p)).flatMap { pred1 =>
                oss.traverse(os => contextualiseOrderTerms(context, table, os)).flatMap { oss0 =>
                  oss0.filterA(os => columnForSqlTerm(context, os.term).map(keyCols.contains)).flatMap { nonKeyOrders =>
                    ...
                    val distOrders = keyCols.map(col => OrderSelection(col.toTerm, true, nullsHigh)) ++ nonKeyOrders
              ...
              pred0.traverse(p => contextualiseWhereTerms(context, baseRef, p)).flatMap { pred1 =>
                oss.traverse(os => contextualiseOrderTerms(context, baseRef, os)).flatMap { oss0 =>
                  oss0.filterA(os => columnForSqlTerm(context, os.term).map(keyCols.contains)).flatMap { nonKeyOrders =>
                    ...
                    val distOrders = keyCols.map(col => OrderSelection(col.derive(baseRef).toTerm, true, nullsHigh)) ++ nonKeyOrders
                ...
                oss.traverse(os => contextualiseOrderTerms(context, table, os)).flatMap { oss0 =>
                  val (nonNullKeys, keyOrder) =
                    offset0.orElse(limit0).map { _ =>
                      ...
                      val keyOrder0 = keyCols.diff(orderCols).map(col => OrderSelection(col.toTerm, true, nullsHigh))
                  ...
                  oss.traverse(os => contextualiseOrderTerms(context, baseRef, os)).flatMap { oss0 =>
                    oss0.filterA(os => columnForSqlTerm(context, os.term).map(keyCols.contains)).flatMap { nonKeyOrders =>
                      ...
                      val distQuery = SqlSelect(
                        ...
                        withs = Nil,
                        table = baseRef,
                        cols = predCols ++ distOrderCols.map(col => distinctOrderColumn(baseRef, col, predCols, distOrders)),
                        joins = (filterJoins ++ orderJoins).distinct,
                        wheres = (pred1 ++ nonNullKeys).distinct,
                        orders = distOrders,
                        offset = None,
                      ...
                      val distName = "dist"
                      val distSub = SubqueryRef(context, distName, distQuery, Laterality.NotLateral)

                      val predCols0 = keyCols.map(_.derive(distSub))
                      oss.traverse(os => contextualiseOrderTerms(context, distSub, os)).flatMap { outerOss0 =>
                        val orders = outerOss0 ++ keyCols.diff(orderCols).map(col => OrderSelection(col.derive(distSub).toTerm, true, nullsHigh))
                        val predQuery = SqlSelect(
                          context = context,
                          withs = Nil,
                          table = distSub,
      ...
      /** Yields an equivalent query encapsulating this query as a subquery */
      def toSubquery(name: String): Result[SqlSelect] = toSubquery(name, Laterality.NotLateral).success
      ...
      def toFragment: Aliased[Fragment] = {
        for {
          ...
          cols0   <- cols.traverse(col => Aliased.liftR(selectedNeedsCollation(col)).flatMap(col.toDefFragment))
          dcols   <- distinct.traverse(col => Aliased.liftR(selectedNeedsCollation(col)).flatMap(col.toRefFragment))
          ...
          off     =  offset.orElse(defaultOffsetForLimit(limit)).map(o => offsetToFragment(Fragments.const(s"$o"))).getOrElse(Fragments.empty)
          lim     =  limit.map(l => limitToFragment(Fragments.const(s"$l"))).getOrElse(Fragments.empty)
          _       <- Aliased.popOwner
        } yield
