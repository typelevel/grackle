...
/**
 * MySQL 8.0.14+ dialect. LATERAL derived tables (8.0.14) and window functions (8.0) are native,
 * so the two deep structural hooks are pg-like; the differences are all local rendering: no
 * NULLS FIRST/LAST (CASE-WHEN emulation, nulls-low polarity), no DISTINCT ON (FirstValueColumn,
 * as MSSQL), no ILIKE (LIKE is already case-insensitive under the default *_ci collations -
 * it's the case-SENSITIVE branch that needs help), and a LIMIT-anchored offset/limit grammar
 * identical to SQLite's comma form, handled the same way.
 */
