...
final class ErrorKeysSuite extends DoobieMySqlDatabaseSuite with SqlErrorKeysSuite {
  lazy val mapping = new DoobieMySqlTestMapping(transactor) with SqlCompositeKeyMapping[IO]
}
...
final class NullableParentSuite extends DoobieMySqlDatabaseSuite with SqlNullableParentSuite {
  lazy val mapping = new DoobieMySqlTestMapping(transactor) with SqlNullableParentMapping[IO]
}

final class NullOrderingSuite extends DoobieMySqlDatabaseSuite with SqlNullOrderingSuite {
  lazy val mapping = new DoobieMySqlTestMapping(transactor) with SqlNullOrderingMapping[IO]
}
...
final class QualifiedNamesSuite extends DoobieMySqlDatabaseSuite with SqlQualifiedNamesSuite {
  lazy val mapping = new DoobieMySqlTestMapping(transactor) with SqlQualifiedNamesMapping[IO]
}

final class TableNameSuite extends DoobieMySqlDatabaseSuite with SqlTableNameSuite {
  lazy val mapping = new DoobieMySqlTestMapping(transactor) with SqlQualifiedNamesMapping[IO]
}
...
final class UnionOrderSuite extends DoobieMySqlDatabaseSuite with SqlUnionOrderSuite {
  lazy val mapping = new DoobieMySqlTestMapping(transactor) with SqlUnionOrderMapping[IO]
}
 ...
 * `Meta[String]`, so `sqlTypeName` reports VARCHAR for it and the JSON case never fires. On
 * MariaDB the same case has to render a bare NULL instead, `CAST(NULL AS JSON)` being a syntax
 * error there, so the two dialects disagree here by design and a test each is what says so.
 */
final class AscribedJsonNullSuite extends DoobieMySqlDatabaseSuite {
  test("ascribed null for a JSON column") {
    ...
    val mapping = new DoobieMySqlTestMapping(transactor) with SqlCompositeKeyMapping[IO]
    val sql = mapping.ascribedNullToFragment((json, false)).internals.sql
    assertEquals(sql.trim, "CAST(NULL AS JSON)")
  }
}
