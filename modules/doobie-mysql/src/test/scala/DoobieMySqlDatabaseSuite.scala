...
import grackle.doobie.mysql.DoobieMySqlMapping
import grackle.doobie.test.DoobieDatabaseSuite
import grackle.sql.test._

trait DoobieMySqlDatabaseSuite extends DoobieDatabaseSuite {
  abstract class DoobieMySqlTestMapping[F[_]: Sync](
      transactor: Transactor[F],
      monitor: DoobieMonitor[F] = DoobieMonitor.noopMonitor[IO])
      extends DoobieMySqlMapping[F](transactor, monitor)
  ...
  def transactorResource: Resource[IO, Transactor[IO]] =
    Resource.pure(
      Transactor.fromDriverManager[IO](
        ...
        // what the fixtures hold, whatever zone the JVM runs in - checked by running the
        // timestamp suites under TZ=Europe/Berlin. allowPublicKeyRetrieval is required for
        // caching_sha2_password auth without SSL.
        // No charset params: Connector/J 8+ already defaults the connection charset to
        // utf8mb4, and fixture bytes are stored correctly (see this module's entrypoint.sh),
        // verified by ablation - the suites stay green and byte-correct without them.
        "jdbc:mysql://localhost:3306/test?connectionTimeZone=UTC&sslMode=DISABLED&allowPublicKeyRetrieval=true",
        "test",
        "test",
        None
      )
  ...
  val transactorFixture: IOFixture[Transactor[IO]] =
    ResourceSuiteLocalFixture("doobiemysql", transactorResource)
