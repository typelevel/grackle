  ...
  def transactorResource: Resource[IO, Transactor[IO]] =
    Resource.pure(
      Transactor.fromDriverManager[IO](
        ...
        // connectionTimeZone pins the session to the same UTC the forked test JVM runs in;
        // allowPublicKeyRetrieval is required for caching_sha2_password auth without SSL.
