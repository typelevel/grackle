...
/**
 * Indicates a position within an abstract data model during the interpretation of a GraphQL
 * query.
 */
trait Cursor {
  ...
  /**
   * Yield the value at this `Cursor` as a value of type `T` if possible, an error or the left
   * hand side otherwise.
   */
  def as[T: ClassTag: TypeName]: Result[T] =
    classTag[T]
      .unapply(focus)
      .toResultOrError(
        s"Expected value of type ${typeName[T]} for focus of type $tpe at path $path, found $focus")
  ...
  /**
   * Yield the value at this `Cursor` rendered as Json if it is of a scalar or enum type, an
   * error or the left hand side otherwise.
   */
  ...
  /**
   * Yield a list of `Cursor`s corresponding to the elements of the value at this `Cursor` if it
   * is of a list type, or an error or the left hand side otherwise.
   */
  final def asList: Result[List[Cursor]] = asList(List)

  /**
   * Yield a collection of `Cursor`s corresponding to the elements of the value at this `Cursor`
   * if it is of a list type, or an error or the left hand side otherwise.
   */
  def asList[C](factory: Factory[Cursor, C]): Result[C]

  /**
   * Yields the number of elements of this `Cursor` if it is of a list type, or an error
   * otherwise.
   */
  ...
  /**
   * Yield an optional `Cursor`s corresponding to the value at this `Cursor` if it is of a
   * nullable type, or an error on the left hand side otherwise. The resulting `Cursor` will be
   * present iff the current value is present in the model.
   */
  def asNullable: Result[Option[Cursor]]

  /**
   * Yields whether or not this `Cursor` is defined if it is of a nullable type, or an error
   * otherwise.
   */
  ...
  /**
   * Yield a `Cursor` corresponding to the value at this `Cursor` narrowed to type `subtpe`, or
   * an error on the left hand side if such a narrowing is not possible.
   */
  def narrow(subtpe: TypeRef): Result[Cursor]

  /**
   * Yield a `Cursor` corresponding to the value of the field `fieldName` of the value at this
   * `Cursor`, or an error on the left hand side if there is no such field.
   */
  def field(fieldName: String, resultName: Option[String]): Result[Cursor]

  /**
   * Yield the value of the field `fieldName` of this `Cursor` as a value of type `T` if
   * possible, an error or the left hand side otherwise.
   */
  ...
  /**
   * Yield a `Cursor` corresponding to the value of the possibly nullable field `fieldName` of
   * the value at this `Cursor`, or an error on the left hand side if there is no such field.
   */
  ...
  /**
   * Yield a `Cursor` corresponding to the value of the field identified by path `fns` starting
   * from the value at this `Cursor`, or an error on the left hand side if there is no such
   * field.
   */
  ...
  /**
   * Yield a list of `Cursor`s corresponding to the values generated by following the path `fns`
   * from the value at this `Cursor`, or an error on the left hand side if there is no such
   * path.
   */
   ...
   * Yield a list of `Cursor`s corresponding to the values generated by following the path `fns`
   * from the value at this `Cursor`, or an error on the left hand side if there is no such
   * path. If the field at the end of the path is a list then yield the concatenation of the
   * lists of cursors corresponding to the field elements.
   */
  def flatListPath(fns: List[String]): Result[List[Cursor]] =
    listPath(fns).flatMap(cs =>
      cs.flatTraverse(c => if (c.isList) c.asList else List(c).success))
}
...
object Cursor {
  ...
  abstract class AbstractCursor extends Cursor {
    ...
    def asLeaf: Result[Json] =
      Result.internalError(
        s"Expected Scalar type, found $tpe for focus ${focus} at ${context.path.reverse.mkString("/")}")
  ...
  class ProxyCursor(underlying: Cursor) extends Cursor {
    ...
    def field(fieldName: String, resultName: Option[String]): Result[Cursor] =
      underlying.field(fieldName, resultName)
  }

  /**
   * Empty `Cursor` with no content
   */
  case class EmptyCursor(context: Context, parent: Option[Cursor], env: Env)
   ...
   */
  case class ListTransformCursor(underlying: Cursor, newSize: Int, newElems: Seq[Cursor])
      extends ProxyCursor(underlying) {
    override def withEnv(env: Env): Cursor =
      new ListTransformCursor(underlying.withEnv(env), newSize, newElems)
  ...
  case class DeferredCursor(
      context: Context,
      parent: Option[Cursor],
      env: Env,
      deferredPath: List[String],
      mkCursor: (Context, Cursor) => Result[Cursor])
      extends AbstractCursor {
    def focus: Any = Result.internalError(s"Empty cursor has no focus")
    def withEnv(env0: Env): DeferredCursor = copy(env = env.add(env0))

    override def field(fieldName: String, resultName: Option[String]): Result[Cursor] =
      if (fieldName != deferredPath.head)
        Result.internalError(s"No field '$fieldName' for type ${tpe.underlying}")
      else
        for {
          ...
          cursor <-
            if (path.sizeCompare(1) > 0)
              DeferredCursor(fieldContext, Some(this), env, deferredPath.tail, mkCursor).success
...
/**
 * Context represents a position in the output tree in terms of, 1) the path through the schema
 * to the position 2) the path through the schema with query aliases applied 3) the type of the
...
case class Context(
  ...
  def forField(fieldName: String, resultName: String): Result[Context] =
    tpe
      .underlyingField(fieldName)
      .map { fieldTpe =>
        copy(
          path = fieldName :: path,
          resultPath = resultName :: resultPath,
          typePath = fieldTpe :: typePath)
      }
  ...
  def forField(fieldName: String, resultName: Option[String]): Result[Context] =
    tpe
      ...
      .map { fieldTpe =>
        copy(
          path = fieldName :: path,
          resultPath = resultName.getOrElse(fieldName) :: resultPath,
          typePath = fieldTpe :: typePath)
      }
      .toResult(s"No field '$fieldName' for type ${tpe.underlying}")
...
object Context {
  def apply(rootTpe: Type, fieldName: String, resultName: Option[String]): Option[Context] = {
    for {
      ...
      rootTpe,
      List(fieldName),
      List(resultName.getOrElse(fieldName)),
      List(tpe))
  }
  ...
  def apply(path: Path): Result[Context] =
    path.path.foldLeftM(Context(path.rootTpe, Nil, Nil, Nil)) {
      case (acc, elem) =>
