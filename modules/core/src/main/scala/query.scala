...
object Query {
  ...
  /**
   * Identifies a component boundary. `join` is applied to the current cursor and `child`
   * yielding a continuation query which will be evaluated by the interpreter identified by
   * `componentId`.
   */
  ...
  /**
   * Embeds possibly batched effects. `handler` is applied to one or more possibly batched
   * queries and cursors yielding corresponding continuation queries and cursors which will be
   * evaluated by the current interpreter in the next phase.
   */
   ...
   */
  case class UntypedInlineFragment(
      tpnme: Option[String],
      directives: List[Directive],
      child: Query)
      extends Query {
  ...
  /**
   * Orders the results of list-producing continuation `child` by fields specified by
   * `selections`.
   */
  ...
  case class OrderSelection[T: Order](
      term: Term[T],
  ...
  def renameRoot(q: Query, rootName: String): Option[Query] = q match {
    ...
    case t @ TransformCursor(_, child) =>
      renameRoot(child, rootName).map(rc => t.copy(child = rc))
    case _ => None
  }
  ...
  /**
   * Yields the supplied query with its the top-level field selection of the supplied replaced
   * with `newChild` if it is unique, `None` otherwise.
   */
      ...
      case paths =>
        ...
        val grouped: List[Query] = multiElemPaths.groupBy(_.head).toList.map {
          case (fieldName, suffixes) =>
            Select(
              fieldName,
              mergeQueries(mkPathQuery(suffixes.map(_.tail).filterNot(_.isEmpty))))
        }
  ...
  private abstract class QueryMerger[T <: Query] {
    ...
    def flattenLevel(qs: List[Query]): List[Query] = {
      ...
      def loop(
          qs: Iterator[Query],
          prevNarrow: Option[(TypeRef, List[Query])],
          acc: List[Query]): List[Query] = {
          ...
          qs.next() match {
            case Narrow(tpe, child) =>
              prevNarrow match {
                ...
                case Some((tpe0, children)) if tpe0.name == tpe.name =>
                  loop(qs, Some((tpe, child :: children)), acc)
  ...
  private object UntypedQueryMerger extends QueryMerger[UntypedSelect] {
    ...
    def groupSelects(qs: List[Query]): Map[(String, Option[String]), Query] = {
      val selects = qs
        .filter {
          case _: UntypedSelect => true
          case _ => false
        }
        .asInstanceOf[List[UntypedSelect]]
      selects.groupBy(sel => (sel.name, sel.alias)).view.mapValues(mergeSelects).toMap
    }
