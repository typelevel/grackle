...
class QueryCompiler(parser: QueryParser, schema: Schema, phases: List[Phase]) {
      ...
      case Some(untypedVars) =>
        untypedVars.asObject match {
          ...
          case Some(obj) =>
            varDefs
              .traverse(iv =>
                checkVarValue(iv, obj(iv.name), "variable values").map(v =>
                  (iv.name, (iv.tpe, v))))
              .map(_.toMap)
        }
    }
  ...
  def validateVariablesAndFragments(
      ops: List[UntypedOperation],
      frags: List[UntypedFragment],
      reportUnused: Boolean): List[Problem] = {
    val (uniqueFrags, duplicateFrags) =
      frags.map(_.name).foldLeft((Set.empty[String], Set.empty[String])) {
    ...
    if (duplicateFrags.nonEmpty)
      ...
      findCycle match {
        ...
        case _ =>
          def validateOp(
              op: UntypedOperation,
              pendingFrags: Set[String]): (List[Problem], Set[String]) = {
            val pendingVars = op.variables.map(_.name).toSet
            val (dqv, dqf) = collectQueryRefs(op.query)

            @tailrec
            def closeRefs(
                pendingFrags: List[String],
                seenVars: Set[String],
                seenFrags: Set[String]): (Set[String], Set[String]) =
              pendingFrags match {
            ...
            val varProblems =
              if (qv == pendingVars) Nil
                ...
                val undefinedProblems =
                  qv.diff(pendingVars)
                    .toList
                    .map(nme => Problem(s"Variable '$nme' is undefined"))
                ...
                val unusedProblems =
                  if (!reportUnused) Nil
                    ...
                    pendingVars
                      .diff(qv)
                      .toList
                      .map(nme => Problem(s"Variable '$nme' is unused"))
            ...
            val fragProblems =
              if (qf.subsetOf(uniqueFrags)) Nil
                ...
                val undefinedProblems =
                  undefined.toList.map(nme => Problem(s"Fragment '$nme' is undefined"))
                undefinedProblems
              }
  ...
  /**
   * Validates that field mergeability rules are satisfied for the supplied operations and
   * fragments.
   *
   * Returns a list of problems encountered.
   */
  def validateFieldMergeability(
      ops: List[UntypedOperation],
      frags: List[UntypedFragment]): List[Problem] = {
    ...
    def validateOp(op: UntypedOperation): List[Problem] = {
      ...
      def collectSelects(
          queries: List[(NamedType, Query)]): List[(NamedType, UntypedSelect)] = {
        queries.flatMap {
      ...
      def checkShapes(
          tpes: List[Type],
          resultName: String): Either[List[Problem], List[NamedType]] = {
        if (tpes.sizeCompare(1) <= 0) Right(tpes.map(_.underlyingNamed))
          ...
          def stripNull(tpes: List[Type]): Either[List[Problem], List[NamedType]] = {
            if (tpes.forall(!_.isNullable)) stripList(tpes)
              ...
              Left(List(Problem(
                s"Cannot merge fields named '$resultName' of both nullable and non-nullable types")))
          }

          def stripList(tpes: List[Type]): Either[List[Problem], List[NamedType]] = {
            if (tpes.forall(!_.isList)) Right(tpes.map(_.underlyingNamed))
            else if (tpes.forall(_.isList))
              stripNull(tpes.collect { case ListType(elem) => elem })
            else
              Left(
                List(Problem(
                  s"Cannot merge fields named '$resultName' of both list and non-list types")))
          }
                ...
                if (tpes.tail.forall(_ =:= first)) r
                  ...
                  val leafErrors =
                    if (leafNames.sizeCompare(1) <= 0) Nil
                      ...
                      List(Problem(
                        s"Cannot merge fields named '$resultName' of distinct leaf types ${leafNames.mkString(", ")}"))
                  val nonLeafErrors =
                    if (nonLeafNames.isEmpty) Nil
                    else
                      List(Problem(
                        s"Cannot merge fields named '$resultName' of leaf types ${leafNames.mkString(", ")} and non-leaf types ${nonLeafNames.mkString(", ")}"))
              ...
              case (resultName, sels) =>
                if (sels.sizeCompare(1) <= 0) Nil
                  ...
                  allTypes.flatMap { tpe =>
                    val conflictSet = sels.collect {
                      case (ntpe, sel) if ntpe <:< tpe || tpe <:< ntpe => sel
                    }
                      ...
                      if (noNameConflicts && noArgConflicts) Nil
                        ...
                        val nameProblems =
                          if (noNameConflicts) Nil
                            ...
                            List(Problem(
                              s"Cannot merge fields with alias '$resultName' and names ${conflictSet.map(s => s"'${s.name}'").distinct.mkString(", ")}"))
                        val argProblems =
                          if (noArgConflicts) Nil
                          else
                            List(Problem(
                              s"Cannot merge fields named '$resultName' with different arguments"))
...
object QueryCompiler {
   ...
   * Elaboration monad.
   *
   * Supports threading of state through the elaboration of a query. Provides, + access to the
   * schema, context, variables and fragments of a query. + ability to transform the children of
   * Selects to supply semantics for field arguments. + ability to add contextual data to the
   * resulting query both to support propagation of context to the elaboration of children, and
   * to to drive run time behaviour. + ability to add selects for additional attributes to the
   * resulting query. + ability to test existence and properties of neighbour nodes of the node
   * being elaborated. + ability to report errors and warnings during elaboration.
   */
  ...
  object Elab {
    ...
    def transformFragments(
        f: Map[String, UntypedFragment] => Elab[Map[String, UntypedFragment]]): Elab[Unit] =
      for {
        fs <- fragments
        fs0 <- f(fs)
    ...
    /**
     * The value bound to the supplied name in the elaboration environment, if any, failing
     * otherwise
     */
    def envE[T: ClassTag: TypeName](nme: String): Elab[T] =
      env(nme).flatMap(v =>
        Elab.liftR(
          v.toResultOrError(s"Key '$nme' of type ${typeName[T]} was not found in $this")))
    ...
    def transformChild(f: Query => Elab[Query]): Elab[Unit] =
      StateT.modify(_.addChildTransform(f))

    /**
     * Applies the supplied transformation to the child of the node currently being elaborated
     */
    def transformChild(f: Query => Query)(implicit dummy: DummyImplicit): Elab[Unit] =
      transformChild(q => Elab.pure(f(q)))

    /**
     * Applies the supplied transformation to the child of the node currently being elaborated
     */
    def transformChild(f: Query => Result[Query])(
        implicit dummy1: DummyImplicit,
        dummy2: DummyImplicit): Elab[Unit] = transformChild(q => Elab.liftR(f(q)))
    ...
    /**
     * Add the supplied attributed and corresponding query, if any, to the query being
     * elaborated
     */
    def addAttribute(name: String, query: Query = Empty): Elab[Unit] =
      StateT.modify(_.addAttribute(name, query))
    ...
    def warning(msg: String): Elab[Unit] =
      StateT(s => Result.warning[(ElabState, Unit)](msg, (s, ())))

    /**
     * Report the supplied GraphQL warning during elaboration
     */
    def warning(err: Problem): Elab[Unit] =
      StateT(s => Result.warning[(ElabState, Unit)](err, (s, ())))
    ...
    def internalError[T](msg: String): Elab[T] =
      StateT(_ => Result.internalError[(ElabState, T)](msg))

    /**
     * Report the supplied internal error during elaboration
     */
    def internalError[T](err: Throwable): Elab[T] =
      StateT(_ => Result.internalError[(ElabState, T)](err))
    ...
    def push(schema: Schema, context: Context, query: Query): Elab[Unit] =
      StateT.modify(_.push(schema, context, query))
  ...
  case class ElabState(
    ...
    def hasSibling(fieldName: String): Boolean =
      parent.exists(s => Query.hasField(s.query, fieldName))
    ...
    def env[T: ClassTag](nme: String): Option[T] =
      localEnv.get(nme).orElse(parent.flatMap(_.env(nme)))
    def addAttribute(name: String, query: Query = Empty): ElabState =
      copy(attributes = (name, query) :: attributes)
    def addChildTransform(f: Query => Elab[Query]): ElabState =
      copy(childTransform = childTransform.andThen(_.flatMap(f)))
    def push: ElabState = copy(
      parent = Some(this),
      localEnv = Env.empty,
      attributes = Nil,
      childTransform = Elab.pure)
  ...
  trait Phase {
      ...
      for {
        ...
        _ <- {
          ...
          if (isLeaf && child != Empty)
            ...
            Elab.failure(
              s"Non-leaf field '$fieldName' of $obj must have a non-empty subselection set")
          else
            Elab.pure(())
        }
      } yield ()
    ...
    val TypenameType = ObjectType(
      s"__Typename",
      None,
      List(Field("__typename", None, Nil, StringType, Nil)),
      Nil,
      Nil)
  }
            ...
            case (nme, f @ UntypedFragment(_, tpnme, _, child)) =>
              for {
                ...
                tpe <- Elab.liftR(
                  Result.fromOption(
                    s.definition(tpnme).orElse(Introspection.schema.definition(tpnme)),
                    s"Unknown type '$tpnme' in fragment definition"))
    ...
    override def transform(query: Query): Elab[Query] =
      query match {
        case s @ UntypedSelect(
              fieldName @ ("__typename" | "__schema" | "__type"),
              _,
              _,
              _,
              _) =>
   ...
   *   2. `skip` and `include` directives are handled during this phase and the guarded
   *      subqueries are retained or removed as appropriate.
   ...
   *   5. verifies that leaves have an empty subselection set and that structured types have a
   *      non-empty subselection set.
   */
        ...
        case sel @ UntypedSelect(fieldName, alias, args, dirs, child) =>
          isSkipped(dirs).ifM(
            ...
            for {
              ...
              eDirs <- Elab.liftR(
                Directive.elaborateDirectives(
                  s,
                  dirs.filterNot(dir => dir.name == "skip" || dir.name == "include"),
                  vars))
              _ <- Elab.push(childCtx, child)
              ec <- transform(child)
              _ <- Elab.pop
        ...
        case UntypedFragmentSpread(nme, dirs) =>
          isSkipped(dirs).ifM(
            ...
            for {
              ...
              subtpe <- Elab.liftR(
                s.definition(f.tpnme)
                  .toResult(s"Unknown type '${f.tpnme}' in type condition of fragment '$nme'"))
              _ <- Elab
                .failure(s"Fragment '$nme' is not compatible with type '${c.tpe}'")
                .whenA(!fragmentApplies(s, subtpe, ctpe))
            ...
            for {
              ...
              subtpe <- tpnme0 match {
                ...
                case Some(tpnme) =>
                  Elab.liftR(
                    s.definition(tpnme)
                      .toResult(s"Unknown type '$tpnme' in type condition of inline fragment"))
              }
              _ <- Elab
                .failure(
                  s"Inline fragment with type condition '$subtpe' is not compatible with type '$ctpe'")
                .whenA(!fragmentApplies(s, subtpe, ctpe))
              _ <- Elab.push(c.asType(subtpe), child)
              ec <- transform(child)
              _ <- Elab.pop
      ...
      value match {
        case VariableRef(varName) =>
          for {
            v <- Elab.vars
            tv <- Elab.liftR(
              Result.fromOption(v.get(varName), s"Variable '$varName' is undefined"))
            b <- tv match {
  ...
  /**
   * A compiler phase which applies GraphQL field merge rules to an untyped query.
   */
   ...
   * This phase,
   *
   *   1. types bindings according to the schema: i) untyped enums are validated and typed
   *      according to their declared type. ii) String and Int bindings are translated to ID
   *      bindings where appropriate. iii) default values are supplied for missing arguments.
   *      iv) arguments are permuted into the order declared in the schema.
   *   2. eliminates Select arguments by delegating to a model-specific `PartialFunction` which
   *      is responsible for translating `Select` nodes into a form which is directly
   *      interpretable, for example, replacing them with a `Filter` or `Unique` node with a
  ...
  trait SelectElaborator extends Phase {
            ...
            field <- obj match {
              case twf: TypeWithFields =>
                Elab.liftR(
                  twf
                    .fieldInfo(fieldName)
                    .toResult(s"No field '$fieldName' for type ${obj.underlying}"))
              case _ => Elab.failure(s"Type $obj is not an object or interface type")
            }
            eArgs <- Elab.liftR(elaborateFieldArgs(obj, field, args))
            _ <-
              if (s eq Introspection.schema)
                elaborateIntrospection(Introspection.schema.uncheckedRef(obj), fieldName, eArgs)
      ...
      (ref, name, args) match {
        case (QueryTypeRef, "__type", List(Binding("name", StringValue(name)))) =>
          Elab.transformChild(child =>
            Unique(Filter(Eql(TypeTypeRef / "name", Const(Option(name))), child)))

        case (
              TypeTypeRef,
              "fields",
              List(Binding("includeDeprecated", BooleanValue(include)))) =>
          Elab.transformChild(child =>
            if (include) child
            else Filter(Eql(FieldTypeRef / "isDeprecated", Const(false)), child))
        case (
              TypeTypeRef,
              "enumValues",
              List(Binding("includeDeprecated", BooleanValue(include)))) =>
          Elab.transformChild(child =>
            if (include) child
            else Filter(Eql(EnumValueTypeRef / "isDeprecated", Const(false)), child))
        case (
              TypeTypeRef,
              "inputFields",
              List(Binding("includeDeprecated", BooleanValue(include)))) =>
          Elab.transformChild(child =>
            if (include) child
              ...
              Filter(Eql(Introspection.__InputValueType / "isDeprecated", Const(false)), child))
        case (
              FieldTypeRef,
              "args",
              List(Binding("includeDeprecated", BooleanValue(include)))) =>
    ...
    def elaborateFieldArgs(
        tpe: NamedType,
        field: Field,
        args: List[Binding]): Result[List[Binding]] = {
      val infos = field.args
      val unknownArgs = args.filterNot(arg => infos.exists(_.name == arg.name))
      if (unknownArgs.nonEmpty)
        Result.failure(
          s"Unknown argument(s) ${unknownArgs.map(s => s"'${s.name}'").mkString("", ", ", "")} in field ${field.name} of type ${tpe.name}")
      else {
        val argMap = args.groupMapReduce(_.name)(_.value)((x, _) => x)
        infos.traverse(info =>
          checkValue(info, argMap.get(info.name), s"field '${field.name}' of type '$tpe'").map(
            v => Binding(info.name, v)))
      }
    }
  }
  ...
  object SelectElaborator {
    ...
    /**
     * Construct a `SelectElaborator` given a partial function which is called for each Select`
     * node in the query.
     */
    def apply(
        sel: PartialFunction[(TypeRef, String, List[Binding]), Elab[Unit]]): SelectElaborator =
   ...
   *
   * This phase transforms the input query by assigning subtrees to component mappings as
   * specified by the supplied `cmapping`.
   *
   * The mapping has `Type` and field name pairs as keys and mapping and join function pairs as
   * values. When the traversal of the input query visits a `Select` node with type
   * `Type.field name` it will replace the `Select` with a `Component` node comprising,
   *
   ...
   *
   * This join function is responsible for computing the continuation query which will be
   * evaluated by the responsible interpreter.
   *
   * Because the join is provided with the cursor of the parent interpreter the subquery can be
   * parameterised with values derived from the parent query.
   */
  class ComponentElaborator[F[_]] private (
      cmapping: Map[(Type, String), (Mapping[F], (Query, Cursor) => Result[Query])])
      extends Phase {
    override def transform(query: Query): Elab[Query] =
      query match {
        case s @ Select(fieldName, resultName, child) =>
          for {
            ...
            obj <- Elab.liftR(
              c.tpe
                .underlyingObject
                .toResultOrError(s"Type ${c.tpe} is not an object or interface type"))
          ...
          } yield cmapping.get((ref, fieldName)) match {
  ...
  object ComponentElaborator {
    ...
    def apply[F[_]](mappings: Seq[ComponentMapping[F]]): ComponentElaborator[F] =
      new ComponentElaborator(
        mappings.map(m => ((m.tpe, m.fieldName), (m.mapping, m.join))).toMap)
  }

  /**
   * A compiler phase which partitions a query for execution which may invoke multiple effect
   * handlers.
   *
   * This phase transforms the input query by assigning subtrees to effect handlers as specified
   * by the supplied `effects`, which takes a `Context` and fieldName, retuning an
   * `EffectHandler` (if any).
   *
   * The mapping has `Type` and field name pairs as keys and effect handlers as values. When the
   * traversal of the input query visits a `Select` node with type `Type.field name` it will
   * replace the `Select` with an `Effect` node comprising,
   *
   *   1. the effect handler which will be responsible for running the effect and evaluating the
   *      subquery against its result.
   *   2. the subquery which will be evaluated by the effect handler.
   */
  class EffectElaborator[F[_]] private (effects: (Context, String) => Option[EffectHandler[F]])
      extends Phase {
    override def transform(query: Query): Elab[Query] =
      query match {
        case s @ Select(fieldName, resultName, child) =>
  ...
  object EffectElaborator {
    ...
    def apply[F[_]](
        effects: (Context, String) => Option[EffectHandler[F]]): EffectElaborator[F] =
