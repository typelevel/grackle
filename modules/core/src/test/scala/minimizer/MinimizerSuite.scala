...
final class MinimizerSuite extends CatsEffectSuite {
  ...
  def run(query: String, expected: String, echo: Boolean = false)(
  ...
  test("minimize simple fragment query") {
    ...
    val expected =
      """query withFragments{user(id:1){friends{...friendFields},mutualFriends{...friendFields}}},fragment friendFields on User{id,name,profilePic}"""
  ...
  test("minimize nested fragment query") {
    ...
    val expected =
      """query withNestedFragments{user(id:1){friends{...friendFields},mutualFriends{...friendFields}}},fragment friendFields on User{id,name,...standardProfilePic},fragment standardProfilePic on User{profilePic}"""
  ...
  test("minimize typed fragment query") {
    ...
    val expected =
      """query FragmentTyping{profiles{id,__typename,...userFragment,...pageFragment}},fragment userFragment on User{name},fragment pageFragment on Page{title}"""
  ...
  test("minimize inline fragment query") {
    ...
    val expected =
      """query inlineFragmentTyping{profiles{id,...on User{name},...on Page{title}}}"""
  ...
  test("minimize typed union fragment query") {
    ...
    val expected =
      """query FragmentUnionTyping{user:user(id:"1"){favourite{__typename,...userFragment,...pageFragment}},page:user(id:"2"){favourite{__typename,...userFragment,...pageFragment}}},fragment userFragment on User{id,name},fragment pageFragment on Page{id,title}"""
  ...
  test("minimize skip/include field") {
    ...
    val expected =
      """query($yup:Boolean,$nope:Boolean){a:field@skip(if:$yup){subfieldA},b:field@skip(if:$nope){subfieldB},c:field@include(if:$yup){subfieldA},d:field@include(if:$nope){subfieldB}}"""
  ...
  test("minimize skip/include fragment spread") {
    ...
    val expected =
      """query($yup:Boolean,$nope:Boolean){a:field{...frag@skip(if:$yup)},b:field{...frag@skip(if:$nope)},c:field{...frag@include(if:$yup)},d:field{...frag@include(if:$nope)}},fragment frag on Value{subfieldA,subfieldB}"""
  ...
  test("minimize fragment spread with nested skip/include") {
    ...
    val expected =
      """query($yup:Boolean,$nope:Boolean){field{...frag}},fragment frag on Value{a:subfieldA@skip(if:$yup),b:subfieldB@skip(if:$nope),c:subfieldA@include(if:$yup),d:subfieldB@include(if:$nope)}"""
  ...
  test("minimize skip/include inline fragment") {
    ...
    val expected =
      """query($yup:Boolean,$nope:Boolean){a:field{...on Value@skip(if:$yup){subfieldA,subfieldB}},b:field{...on Value@skip(if:$nope){subfieldA,subfieldB}},c:field{...on Value@include(if:$yup){subfieldA,subfieldB}},d:field{...on Value@include(if:$nope){subfieldA,subfieldB}}}"""
  ...
  test("minimize inline fragment with nested skip/include") {
    ...
    val expected =
      """query($yup:Boolean,$nope:Boolean){field{...on Value{a:subfieldA@skip(if:$yup),b:subfieldB@skip(if:$nope),c:subfieldA@include(if:$yup),d:subfieldB@include(if:$nope)}}}"""
  ...
  test("minimize null value") {
    ...
    val expected =
      """query{field{subfield},field(arg:null){subfield},field(arg:23){subfield}}"""
  ...
  test("minimize list value") {
    ...
    val expected =
      """query{listField(arg:[]){subfield},listField(arg:["foo","bar"]){subfield}}"""
  ...
  test("minimize query with UUID argument and custom scalar results") {
    ...
    val expected =
      """query{movieById(id:"6a7837fc-b463-4d32-b628-0f4b3065cb21"){id,title,genre,releaseDate,showTime,nextShowing,duration}}"""
