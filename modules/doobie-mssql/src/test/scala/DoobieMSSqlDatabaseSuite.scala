...
import grackle.doobie.mssql.DoobieMSSqlMapping
...
trait DoobieMSSqlDatabaseSuite extends DoobieDatabaseSuite {
  abstract class DoobieMSSqlTestMapping[F[_]: Sync](
      transactor: Transactor[F],
      monitor: DoobieMonitor[F] = DoobieMonitor.noopMonitor[IO])
      extends DoobieMSSqlMapping[F](transactor, monitor)
  ...
  case class MSSqlConnectionInfo(host: String, port: Int) {
    ...
    val jdbcUrl =
      s"jdbc:sqlserver://$host:$port;databaseName=$databaseName;user=$username;password=$password;trustServerCertificate=true;sendTimeAsDatetime=false;"
  }
  ...
  val transactorFixture: IOFixture[Transactor[IO]] =
    ResourceSuiteLocalFixture("mssqlpg", transactorResource)
