...
//   (testdata/mssql/null-ordering.sql) with NULLs in the ordered column.
...
//
// SQL Server sorts NULLs low (first in ASC, last in DESC) and has no NULLS FIRST/LAST syntax, so
// the two cases requesting the opposite placement only work if the dialect emits a sort key that
// relocates them.
...
final class NullOrderingSuite extends DoobieMSSqlDatabaseSuite {
  lazy val mapping = new DoobieMSSqlTestMapping(transactor) with NullOrderingMapping[IO]
  ...
  test("ascending, nulls last (non-default for SQL Server)") {
    assertIO(surviving(ascending = true, nullsLast = true), List(3, 1))
  }

  test("descending, nulls first (non-default for SQL Server)") {
    assertIO(surviving(ascending = false, nullsLast = false), List(2, 4))
  }

  test("ascending, nulls first (SQL Server's default placement)") {
    assertIO(surviving(ascending = true, nullsLast = false), List(2, 4))
  }

  test("descending, nulls last (SQL Server's default placement)") {
